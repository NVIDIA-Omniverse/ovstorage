...
//!
//! FOR TESTS ONLY. The module is `#[doc(hidden)] pub` so external
//! integration tests — the conformance suite in particular — can drive
//! real data-op round-trips against canned omni1 frames via
//! [`crate::__test_only_backend_with_mock`]; it carries no stability
//! promise.
...
/// `Arc` wrapper implementing [`Transport`] so the handle installed inside
/// `RuntimeOps` shares queue and request-recorder state with the test-side
/// inspector.
