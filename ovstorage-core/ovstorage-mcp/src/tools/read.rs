...
use ovstorage::{ByteRange, Error, ErrorCode, ListOptions, ObjectInfo, ReadOptions, StatOptions};
use rmcp::ErrorData;
...
///   destination's current etag matches `<s>` (`PreconditionFailed`
///   otherwise).
...
impl OvstorageServer {
    ...
    pub async fn ovstorage_capabilities(
        ...
        let outcome = async {
            ...
            let caps = self.stack().capabilities_for(&prefix, None).await?;
            Ok(CapabilitiesResult {
                ...
                supports_rename: caps.supports_rename,
    ...
    pub async fn ovstorage_read(
        ...
        let outcome = async {
            ...
            let (bytes, info) = self.stack().read_bytes(addr, opts, None).await?;
            Ok(ReadResult {
