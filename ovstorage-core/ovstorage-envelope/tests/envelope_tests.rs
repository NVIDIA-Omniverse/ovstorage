...
fn envelope_error_from_rust_error_preserves_code_and_next_action() {
    let rust_err = Error::new(ErrorCode::NotConfigured, "backend kind is not registered")
        .with_next_action("Load the backend plugin into the host plugin registry.");
    ...
    assert_eq!(
        ...
        Some("Load the backend plugin into the host plugin registry.")
    );
...
fn envelope_failure_with_rust_error_round_trips() {
    let rust_err = Error::new(ErrorCode::NoRoute, "no route matches address")
        .with_next_action("Add a matching connection to the active Stack.");
    ...
    assert_eq!(
        ...
        Some("Add a matching connection to the active Stack.")
    );
