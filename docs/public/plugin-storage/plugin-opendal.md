...
`ovstorage-cloud/crates/ovstorage-plugin-opendal/` and compiles as a
...
`0.50`) behind the same `Factory` / `Backend` SPI as the other
...
the ovstorage adapter can prove the SPI behaviour end-to-end.
...
route prefix is matched with `address::is_prefix_of`, and the OpenDAL
key is derived from the percent-decoded path via `address::key`, so
...
compatibility remains the native first-party plugin's job unless the
...
The plugin maps each `Backend` SPI call onto OpenDAL's `Operator`
API.

| SPI method | OpenDAL Operator call |
|---|---|
...
| `write` (Body::Bytes / LocalFile) | `Operator::write_with(...)` → `WriteStep::Done(WriteResult)` directly |
| `write_stream` | `Operator::writer_with(...)` + per-chunk `write(...)` + `close()` — each `BodyStream` chunk goes straight into the writer, no full-payload buffering at the host |
| `write_redirect` | Drivers whose `Capability::presign_write` is true (S3-compatible today) emit a single presigned `PUT` `WriteRedirect`; other drivers return typed `Unsupported`. Requires `WriteOptions.size_hint`. `IfDestExists::Fail` and non-empty `user_metadata` return `Unsupported` on the redirect path so the dispatcher can fall through. Presign lifetime is fixed at 5 minutes. |
...
| `delete_directory` (flat profile: `s3`) | Removes the directory marker only (flat-backend SPI contract). |
...
streaming-writes invariant). Both `Operator` and the `StorageBackend`
SPI are async, so the adapter forwards each call directly with
...
adapter can prove the SPI behaviour end-to-end.
...
`Capability::write_with_if_none_match` is true (S3-compatible
today). `writer_with` and `presign_write_with` do not expose
...
plugin enforces `ReadOptions::if_match` / `DeleteOptions::if_match`
/ `UpdateMetadataOptions::if_match` via post-stat compare against
...
at the SPI boundary before any OpenDAL operator call.
