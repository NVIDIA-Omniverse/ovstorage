...
pub(crate) struct AddConnectionBody {
    /// Backend kind from `GET /v1/backend-kinds`.
    pub backend_kind: String,
    /// Backend-specific config (string/bool/integer values).
    #[serde(default)]
    #[schema(value_type = Object)]
    pub config: HashMap<String, serde_json::Value>,
    /// Backend-specific credentials, stored in the secret store.
    #[serde(default)]
    #[schema(value_type = Object)]
    pub credentials: HashMap<String, String>,
    /// Persist this connection across restarts.
    #[serde(default)]
    pub persist: bool,
    /// Operator-supplied human-readable label.
    #[serde(default)]
...
/// One connection currently registered with the gateway.
#[derive(Serialize, ToSchema)]
pub(crate) struct ConnectionResponse {
    pub id: String,
    pub backend_kind: String,
    pub display_name: String,
    /// Address roots this connection contributes.
    pub current_addresses: Vec<String>,
}

/// Request body for `POST /v1/aliases`.
#[derive(Deserialize, ToSchema)]
pub(crate) struct AddAliasBody {
    /// Caller-facing alias address.
    pub from: String,
    /// Target address the alias resolves to.
    pub to: String,
    /// One of `visible` (default), `hidden`, `suppressed`.
    #[serde(default)]
...
/// One alias currently registered.
#[derive(Serialize, ToSchema)]
...
/// Request body for `PUT /v1/address-visibility`.
#[derive(Deserialize, ToSchema)]
pub(crate) struct SetVisibilityBody {
    pub address: String,
    /// One of `visible`, `hidden`, `suppressed`.
    pub visibility: String,
    #[serde(default)]
    pub persist: bool,
}

/// One visibility override currently in effect.
#[derive(Serialize, ToSchema)]
pub(crate) struct VisibilityOverrideResponse {
    pub address: String,
    pub visibility: String,
    pub persisted: bool,
}
...
/// Returned by `GET /v1/connections`.
#[derive(Serialize, ToSchema)]
pub(crate) struct ConnectionList {
    pub items: Vec<ConnectionResponse>,
}

/// Returned by `GET /v1/aliases`.
#[derive(Serialize, ToSchema)]
pub(crate) struct AliasList {
    pub items: Vec<AliasResponse>,
}

/// Returned by `GET /v1/address-visibility`.
#[derive(Serialize, ToSchema)]
pub(crate) struct VisibilityList {
    pub items: Vec<VisibilityOverrideResponse>,
}
...
/// One event from `POST /v1/connections:authenticate`; `succeeded`,
/// `failed`, and `cancelled` are terminal.
#[derive(Serialize, ToSchema)]
#[serde(tag = "type", rename_all = "snake_case")]
pub(crate) enum AuthEventResponse {
    /// Visit `url` in a browser to complete authentication.
    OpenBrowser { url: String },
    /// Device-code flow: show `user_code` and `verification_url`.
    DeviceCode {
        user_code: String,
    ...
    /// Auth completed.
    Succeeded { connection: ConnectionResponse },
    /// Auth failed.
    Failed { error: ErrorBody },
    /// Caller cancelled the flow.
    Cancelled,
}
