...
/// Resolve a request bundle:
/// a `file_path` (+ optional `profile`) field routes through the AWS shared
/// credentials file, otherwise the explicit key fields are read from the
/// bundle. `Ok(None)` = no credentials supplied (anonymous connection).
pub fn resolve_bundle(bundle: &SecretBundle) -> Result<Option<AwsCredentials>> {
    if let Some(path) = string_field(bundle, "file_path")? {
        let profile = string_field(bundle, "profile")?.unwrap_or_else(|| "default".to_string());
        return Ok(Some(from_aws_credentials_file(&path, &profile)?));
    }
    from_bundle(bundle)
}

/// Read a UTF-8 text field from the bundle; empty/whitespace-only is `None`.
pub(crate) fn string_field(bundle: &SecretBundle, key: &str) -> Result<Option<String>> {
    let Some(value) = bundle.fields.get(key) else {
        return Ok(None);
    };
...
/// Reject bundle keys outside the descriptor's credential schema.
pub(crate) fn validate_credential_fields(bundle: &SecretBundle) -> Result<()> {
    for key in bundle.fields.keys() {
        if !known_credential_field(key) {
            return Err(Error::new(
                ErrorCode::InvalidArgument,
