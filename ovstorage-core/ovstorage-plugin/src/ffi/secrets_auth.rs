...
impl SecretBytes {
    /// Zeroize the buffer in place, leaving it allocated and owned.
    ///
    /// [`Drop`] calls this; it is separate so the wipe is observable
    /// without also releasing the buffer it just cleared.
    pub(crate) fn wipe(&mut self) {
        use zeroize::Zeroize as _;
        if self.bytes.ptr.is_null() {
            return;
        }
        // SAFETY: `bytes` owns `len` initialized bytes for as long as it is
        // alive, and this neither frees nor reallocates them.
        unsafe { std::slice::from_raw_parts_mut(self.bytes.ptr, self.bytes.len).zeroize() };
    }
}

impl Drop for SecretBytes {
    fn drop(&mut self) {
        // Field drop glue runs `Bytes: Drop` right after this, releasing the
        // buffer this call just cleared.
        self.wipe();
        #[cfg(test)]
        released_secrets::record(self);
    }
}

/// Test-only witness for the wipe, recorded at the one moment it can be
/// observed soundly: after [`SecretBytes::wipe`] and before the field drop
/// glue frees the buffer, while this value still owns it.
///
/// The alternative — reading the block back after release — means reading
/// freed and then uninitialized storage, and depends on the allocator
/// handing the same address back without scribbling on it. Recording here
/// needs none of that.
///
/// Thread-local, so a test observes only the secrets its own thread
/// released and the harness can keep running tests in parallel.
#[cfg(test)]
pub(crate) mod released_secrets {
    ...
    pub(crate) fn record(secret: &super::SecretBytes) {
        if secret.bytes.ptr.is_null() {
            return;
        }
        // SAFETY: called from `SecretBytes::drop` before the buffer is
        // released, so it is still owned, allocated, and initialized.
        let contents =
            unsafe { std::slice::from_raw_parts(secret.bytes.ptr, secret.bytes.len).to_vec() };
        let _ = RELEASED.try_with(|released| released.borrow_mut().push(contents));
    }

    /// Drain what this thread has released since the last call.
