        ...
        match err {
            CredentialError::Unavailable { details } => {
                Error::new(ErrorCode::CredentialUnavailable, details).with_next_action(
                    "Update the connection credentials through the Stack, or run the \
                 connection's interactive authentication flow.",
                )
            }
...
/// Closure-driven provider for external token management. Hosts retain the
/// provider in their credential resolver chain.
