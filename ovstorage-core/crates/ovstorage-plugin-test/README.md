...
- `plugin-conformance` proves that the host (`ovstorage`, `ovstorage-broker`, `broker-protocol`, and broker authz plugins) consumes every legal SPI shape correctly.
...
The manifest carries `test_only: bool`; the host loader
(`HostPlugin::load`) refuses any `test_only = true` cdylib unless
`LibraryBuilder::allow_test_plugins(true)` opted in. Refusal surfaces
as `ErrorCode::PluginRejected`. The `ovstorage-plugin-test` crate's
`Cargo.toml` carries `publish = false`; its
`ovstorage_plugin!(TestFactory::default, test_only)` macro invocation
(the function-like macro from
...
emits the manifest with that flag set. There is no
...
- **Manifest**: `name = "ovstorage-plugin-test"` (matches the crate name; the macro pulls it from `CARGO_PKG_NAME`), `version = "0.1.0"`, `test_only = true`. There is **no** `plugin_kind` or `schemes` field on `PluginManifestV1` — see [ovstorage-plugin § C-ABI surface](../ovstorage-plugin/README.md#c-abi-surface).
...
- **Package policy**: workspace member, `publish = false`. The cdylib ships in the release archive's `plugins/` directory so downstream host authors can drive their host through the conformance fixture; the host-level `allow_test_plugins` gate carries the safety story (bundle exclusion is intentionally not a layer — see Threat model below).
- **Credential keys**: none. The plugin never accepts long-lived credentials, cloud keys, OAuth refresh tokens, or bearer tokens.
- **Host gate**: the library and broker load the plugin only when the host opts in via `allow_test_plugins = true`. `Library::load_plugin` (direct, by-path) returns `PluginRejected` when the host has not opted in. `Library::load_plugins_from_dir` (bulk discovery — used by the broker and REST gateway at startup) skips the plugin at debug-log level when the host has not opted in, so a default-posture deployment can scan a `plugins/` directory containing the bundled cdylib without failing startup.
...
`plugin-conformance` tests the host/plugin boundary, not the current spelling of the public `Library` API. Scenario groups are therefore classified by the boundary they exercise:

- **`shim::Backend` object SPI**: calls that dispatch to a configured `Backend` (`stat`, `read`, `write`, `continue_write`, `delete`, `list`, `list_versions`, `copy`, `rename`, directory operations, metadata patch, access check, watch_directory).
- **`shim::Backend` introspection SPI**: `address_roots`, `watch_address_roots`, and immutable `Capabilities`.
- **`shim::Backend` lifecycle SPI**: `probe`, `update_credentials`, stream/delegate destruction, cancellation, and backend teardown.
- **`shim::Factory` SPI**: `descriptor`, `instantiate`, and `authenticate`.
...
| `metadata_rewrite_only` | Advertises `supports_metadata_rewrite_emulation = true` and native patch false; used to verify `allow_rewrite_emulation`. |
...
| `server_side_copy` | Advertises `supports_server_side_copy = true`; copy scenarios expect a single `StorageBackend::copy` call instead of host read/write fallback. |
| `server_side_rename` | Advertises `supports_server_side_rename = true`; rename scenarios expect a single `StorageBackend::rename` call. |
...
1. **The backend plugin** loaded through [ovstorage-plugin](../ovstorage-plugin/README.md). It implements the manifest, `StorageBackendFactory`, `StorageBackendFactory::authenticate`, and `StorageBackend`.
...
Likewise, host authz decisions are not made by backend plugins. Host authorization scenarios use `ovstorage-authz-toml` or a dedicated authz test plugin; `plugin-conformance` only supplies the backend behavior the host dispatches to after authz allows the call.
...
| Scenario | Contract |
|---|---|
| `factory-descriptor-schema-roundtrip` | `StorageBackendFactory::descriptor` exposes config and credential schema; host renders it, validates exact keys, and instantiates from a matching blob. |
| `factory-invalid-config` | Missing, unknown, or wrong-typed config keys fail with the documented `ConnectionError` variant before a `StorageBackend` is installed. |
...
| Scenario | Contract |
|---|---|
| `write-done-inline` | `StorageBackend::write` consumes the body and returns `WriteStep::Done` directly. |
...
| `copy-same-backend-spi` | `server_side_copy` profile causes host to dispatch same-backend copy to `StorageBackend::copy`; plugin returns a `WriteResult` without the host round-tripping bytes. |
...
| `rename-server-side-spi` | `server_side_rename` profile receives one `StorageBackend::rename` call; non-atomic backends must report the partial-success risk through the documented error path. |
| `rename-atomic-spi` | `atomic_rename` profile receives one `StorageBackend::rename` call and either fully moves the object or returns an error without partial state. |
...
| `directory-address-trailing-slash-equivalence` | Host calls `create_directory`, `delete_directory`, and `list` with and without the trailing slash; the plugin observes the same canonical slash-form target for each directory-facing call. |
...
| `stat-list-backed-object-hit` | Unversioned `stat("dir/a")` under a route with `supports_list = true` and `wants_list_backed_stat = true` is satisfied from one parent `StorageBackend::list("dir/")` object entry; a following `stat("dir/b")` reuses the cached list without another backend call. |
| `stat-list-backed-provider-opt-out` | A route with `supports_list = true` and `wants_list_backed_stat = false` dispatches exact `StorageBackend::stat` and never probes the parent list for public `stat`; this pins the `file` plugin's cheap-native-stat behavior. |
| `stat-list-backed-version-selector-bypass` | `stat("dir/a?versionId=1")`, Nucleus-style `stat("dir/a.usd?&3")`, and any other query/fragment-selected address bypass the parent-list cache and dispatch exact `StorageBackend::stat`, because versioned object URLs do not appear in unversioned list entries. |
| `stat-list-backed-list-miss-fallback` | If the parent list succeeds but does not contain the requested object, the host falls back to exact `StorageBackend::stat` rather than treating the list miss as authoritative `NotFound`. |
...
| `metadata-unsupported-not-called` | A profile advertising no patch capability must not receive `StorageBackend::update_metadata`. |
| `capabilities-immutable` | The `Capabilities` value the host stamps on the route at `Factory::instantiate` time stays constant for the backend instance's lifetime, even after writes, auth refresh, or scenario changes. |
...
- [ovstorage-plugin](../ovstorage-plugin/README.md) for the backend SPI, manifest, redirect vocabulary, `StorageBackendFactory`, and `AuthEvent` surface.
...
> Of the defense-in-depth claims below, the implemented layers are: the manifest carries `test_only: true` (`ovstorage_plugin!(TestFactory::default, test_only)` in `src/lib.rs`), the host `LibraryBuilder` gates loading behind `allow_test_plugins(true)` (verified by `dlopen_test_plugin_is_rejected_without_allow_flag`), `Library::load_plugins_from_dir` skips the cdylib silently when the host has not opted in (verified by `bulk_load_skips_test_plugin_without_allow_flag`), and `Cargo.toml` carries `publish = false`. The responder enforces loopback-only binds at `Responder::start`. Bundle exclusion is no longer a defense layer — the cdylib ships in the release archive so downstream host authors can opt in to it. **Unimplemented:** `scratch_dir` namespace (no per-instance scratch directory plumbing); responder-side body verification against `BodySource` / `ResponseParsing` / `ResultCapture` descriptors.
...
- `Library::load_plugin` returns `PluginRejected` when the host has not called `allow_test_plugins(true)`.
- `Library::load_plugins_from_dir` skips the cdylib at debug-log level when the host has not opted in, so a default-posture broker / REST gateway can sweep a `plugins/` directory containing the bundled cdylib without failing startup or surfacing a backend that responds to `test://` URLs.
...
- Every registry entry declares `spi_methods`, `required_profile`, `required_capabilities`, `required_config`, `allowed_hosts`, `expected_calls`, and `failure_contract`.
...
- Negative capability scenarios assert non-dispatch through the call recorder. For example, `metadata-unsupported-not-called` fails if the host calls `StorageBackend::update_metadata` and then translates `Unsupported`; the contract is that the capability check happens before dispatch on host paths that can check it.
...
**Capability gating**
- For every profile, the per-route `Capabilities` value populated at `Factory::instantiate` time matches the documented bits and does not change for the lifetime of the backend instance.
- Negative recorder assertions verify that hosts do not call unsupported SPI methods after checking capabilities.
- Capability-dependent public helpers are tested through host suites, not by adding new plugin methods. Example: a public `materialize` test may use `read-local-delegate`, but the plugin still only implements `StorageBackend::read`.
...
- Panic/exception containment tests run with the same ABI shims production plugins use.
...
- Brokered `LocalDelegate` scenarios never expose broker-local paths to the library.
- Broker-specific scenarios assert broker behavior at the broker boundary, not inside the plugin. The plugin records the same `StorageBackend` calls it would in Direct mode unless the scenario is explicitly about `Host::is_broker` admissibility.
...
**Why this mitigation is sound.** The plugin's manifest carries a `test_only: true` flag set by `ovstorage_plugin!(TestFactory::default, test_only)` (the function-like macro's optional trailing flag — there is no attribute-macro spelling). The host's `LibraryBuilder::allow_test_plugins` is `false` by default; direct `load_plugin` against a `test_only` cdylib returns `ErrorCode::PluginRejected`, and bulk discovery silently skips it. The cdylib ships in the release archive so consumer-side host authors can opt in; bundle exclusion is intentionally not a separate defense layer because (a) the host gate is sufficient — a default-posture host treats the bundled cdylib as unloadable, identical to its being absent, and (b) shipping it makes consumer-side conformance testing possible without rebuilding the plugin out-of-tree.
...
- The function-like macro's optional `, test_only` flag (`ovstorage_plugin!(MyFactory::default, test_only)`) sets `manifest.test_only = true`; the flag is absent by default. There is no attribute-macro spelling.
- Production hosts (broker binary, library outside `#[cfg(test)]`) check `manifest.test_only` at every plugin load. `load_plugin` rejects with `ErrorCode::PluginRejected`. `load_plugins_from_dir` skips the cdylib at debug-log level so default-posture deployments don't crash on a `plugins/` directory containing a bundled test cdylib.
- Operators that want the test plugin actually loaded set `OVSTORAGE_ALLOW_TEST_PLUGINS=1` for the broker (or call `LibraryBuilder::allow_test_plugins(true)` directly in their host code); the default-unset path treats the cdylib as silently absent.

**Verification.**

- `dlopen_test_plugin_is_rejected_without_allow_flag` (`tests/loaded.rs`): direct `load_plugin` against the test cdylib with `allow_test_plugins` off returns `PluginRejected`.
- `bulk_load_skips_test_plugin_without_allow_flag` (`tests/loaded.rs`): default-posture library builds, copies the test cdylib into a tempdir, and confirms `load_plugins_from_dir` returns `Ok(())` while the direct path on the same file still surfaces `PluginRejected`.
