...
/// minutes-long sign-in poll to terminate) — EXCEPT the terminal
/// `Succeeded`, which the caller sends after installing the returned
/// session: a host that reacts to `Succeeded` (e.g. the `ConnectionSet`
/// adapter's `on_authenticated`) must find the session already live, or a
/// completed sign-in would re-handshake an interactive-marker bundle and
/// park. On success the returned `(HandshakeOutput, Connection)` is the
/// session state to install plus the connection to echo in that terminal
/// event; every failure arm pushes its own terminal event and returns
/// `None`.
...
) -> Option<(HandshakeOutput, ovstorage_plugin::Connection)> {
    let span =
        ...
        InteractiveOutcome::Authenticated(tokens_auth) => {
            ...
            .await
            {
                Ok(output) => {
                    ...
                    // The caller installs `output` and THEN sends the
                    // terminal `Succeeded` with this connection.
                    Some((output, connection))
                }
                Err(error) => {
...
/// factory when a prior process persisted a refresh_token in the secret store.
/// On any error, the caller should fall through to interactive sign-in
/// (and clear the stored secret on `AuthRequired`/`AuthExpired`/`PermissionDenied`).
...
/// `connection_url` carrying `access_token` as a query parameter.
///
/// The token is placed with `query_pairs_mut`, matching how the auth flow
/// builds its login URL, rather than by string concatenation. Two properties
/// follow from that and neither holds for a concatenated URL:
///
/// - The value is percent-encoded, so a token containing `&` or `#` cannot
///   introduce further parameters. `connection_url` comes from the discovery
///   response and the token from the Tokens service, so both are chosen by the
///   server; concatenation lets either steer the other's parsing.
/// - The parameter lands in the query even when `connection_url` carries a
///   fragment. Appending after a `#` would put it in the fragment, which is
///   never sent, so the parameter would silently not reach the server at all.
///
/// The scheme is checked for the same reason the auth flow checks it: the URL
/// is server-supplied, and a websocket connect is the wrong place to discover
/// that it is not one.
fn connect_url_with_token(connection_url: &str, access_token: &str) -> Result<String> {
    let mut parsed = Url::parse(connection_url).map_err(|err| {
        Error::new(
            ErrorCode::InvalidArgument,
            format!("Nucleus connection URL is not a valid URL: {err}"),
        )
    })?;
    ...
    if scheme != "ws" && scheme != "wss" {
        return Err(Error::new(
            ErrorCode::InvalidArgument,
            format!("Nucleus connection URL has unsupported scheme {scheme:?}; expected ws or wss"),
        ));
    }
    parsed
        .query_pairs_mut()
...
async fn complete_session(
    ...
    // Some deployments put a reverse proxy in front of Nucleus that rejects
    // requests carrying no valid token, added to protect deployments reachable
    // from the public web against DDoS. Such a proxy has to rule on the HTTP
    // request that opens the websocket, before any frame exists, so the token
    // has to be readable in the URL query. `authorize_token` below is the check
    // Nucleus itself applies; it runs after the upgrade and cannot answer for
    // the edge.
    let connect_url = connect_url_with_token(connection_url, &access_token)?;
...
mod tests {
    ...
    /// A stand-in for a token. Not a credential in any format; its job is to be
    /// a string whose delimiters would break a concatenated URL.
    const HOSTILE_TOKEN: &str = "head&injected=1#fragment";

    #[test]
    fn the_token_lands_in_the_query() {
        let url = connect_url_with_token("ws://h:3019/connection", "plain").unwrap();
        assert_eq!(url, "ws://h:3019/connection?access_token=plain");
    }

    #[test]
    fn an_existing_query_parameter_is_preserved() {
        let url = connect_url_with_token("ws://h/connection?v=2", "plain").unwrap();
        assert_eq!(url, "ws://h/connection?v=2&access_token=plain");
    }

    /// Concatenation lets a token's own `&` and `#` introduce parameters and a
    /// fragment into the client's connect URL. Encoding is what forecloses it.
    #[test]
    fn a_token_cannot_introduce_further_parameters() {
        let url = connect_url_with_token("ws://h/connection", HOSTILE_TOKEN).unwrap();
        let parsed = Url::parse(&url).unwrap();
        let pairs: Vec<_> = parsed.query_pairs().collect();
        assert_eq!(pairs.len(), 1, "extra parameters appeared in {url}");
        assert_eq!(pairs[0].0, "access_token");
        assert_eq!(pairs[0].1, HOSTILE_TOKEN, "the token must survive intact");
        assert_eq!(parsed.fragment(), None, "a fragment appeared in {url}");
    }

    /// Appending after a `#` would put the parameter in the fragment, which is
    /// never sent, so the server would not receive it at all.
    #[test]
    fn a_fragment_does_not_capture_the_token() {
        let url = connect_url_with_token("ws://h/connection#frag", "plain").unwrap();
        let parsed = Url::parse(&url).unwrap();
        assert_eq!(
            parsed.query_pairs().next().unwrap().0,
            "access_token",
            "the token missed the query in {url}"
        );
    }

    #[test]
    fn a_non_websocket_scheme_is_refused() {
        let err = connect_url_with_token("file:///etc/passwd", "plain").unwrap_err();
        assert_eq!(err.code(), ErrorCode::InvalidArgument);
    }

    #[test]
    fn an_unparseable_connection_url_is_refused() {
        let err = connect_url_with_token("not a url", "plain").unwrap_err();
        assert_eq!(err.code(), ErrorCode::InvalidArgument);
    }
    /// `Url::parse` normalises, and the connection URL is what a deployment
    /// advertised. Anything it rewrites changes the request the server
    /// receives, so the shapes discovery actually produces are pinned here.
    #[test]
    fn a_discovery_shaped_url_is_not_rewritten() {
        for base in [
            "ws://host:3019/connection",
            "wss://host:3019/omni/connection",
            "ws://127.0.0.1:3019/connection",
            "ws://[::1]:3019/connection",
            "wss://host:3019/connection?v=2",
        ] {
            let built = connect_url_with_token(base, "T").unwrap();
            let expected_prefix = if base.contains('?') {
                format!("{base}&")
            } else {
                format!("{base}?")
            };
            assert_eq!(
                built,
                format!("{expected_prefix}access_token=T"),
                "`{base}` was rewritten"
            );
        }
    }

    /// Default ports and an empty path are the two rewrites `Url` performs
    /// that a reader would not expect. Asserted so they stay known.
    #[test]
    fn default_ports_and_empty_paths_are_normalised() {
        assert_eq!(
            connect_url_with_token("ws://host:80/p", "T").unwrap(),
            "ws://host/p?access_token=T"
        );
        assert_eq!(
            connect_url_with_token("wss://host:443/p", "T").unwrap(),
            "wss://host/p?access_token=T"
        );
        assert_eq!(
            connect_url_with_token("ws://host:3019", "T").unwrap(),
            "ws://host:3019/?access_token=T"
        );
    }
}
