...
//! RFC-0066: the shipped `ovstorage-mcp.toml` template declares the
//! canonical MCP file-backend stack. It must parse through
...
async fn shipped_default_config_builds_nonempty_stack() {
    let cfg = StackConfig::from_toml_str(include_str!("../ovstorage-mcp.toml"))
        .expect("shipped ovstorage-mcp.toml must parse");
