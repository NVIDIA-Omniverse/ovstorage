...
Cdylib backend Layer for the `broker` kind: forwards every
`Layer` operation across the client <-> broker gRPC
...
- **`src/lib.rs`** — module wiring, `ovstorage_layer_plugin!` macro
  invocation, client helpers, and the `TonicBrokerClient`
  `BrokerClientTransport` implementation (two-layer channel cache via
  `tokio::sync::OnceCell`).
- **`src/layer.rs`** — `BrokerClientLayerFactory`,
  `BrokerClientLayer`, and the `impl Layer` request-envelope bridge.
...
Capabilities are root-scoped. The authoritative profile is forwarded
from the broker's `ListAddressRoots` response into each `RootInfo`
(the proto's `pb::AddressRoot.capabilities` mirrors the upstream plugin's
