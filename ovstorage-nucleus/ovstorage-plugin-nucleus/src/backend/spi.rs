...
//! `NucleusBackend` operations plus the
...
use ovstorage_plugin::{
    ...
    RedirectBodySource, RedirectCredential, RedirectResultBatch, RedirectScope, ResponseParsing,
    ResultCapture, WriteRedirect, WriteRedirectBatch, WriteStep, race_cancel,
...
/// The directory form of a Nucleus path: one trailing separator.
///
/// **A directory verb derives this itself**, as every other backend in the tree
/// now does. `x` and `x/` name one node, so the host does not rewrite a
/// directory-verb address to match the slot — choosing for the backend would be
/// choosing which object a `delete_directory` destroys. Nucleus needs it more
/// sharply than the flat stores do: `delete2` takes the path verbatim, so a
/// slashless `delete_directory("omniverse://srv/docs")` would target the FILE
/// `/docs` rather than the folder `/docs/`, and `list2` would return the
/// contents of a sibling named `docsx` as well.
///
/// The root is already `/` and is left alone; appending would make `//`.
...
impl NucleusBackend {
    ...
    /// The live session cell, for layer-side lifecycle actions that must
    /// reach past the data-path surface (the failed-rotation session clear).
    pub(crate) fn shared(&self) -> &Arc<NucleusShared> {
        &self.shared
    }
    ...
    pub(crate) async fn commit_buffered(
        &self,
        address: Url,
    ...
    pub(crate) async fn finalize_with_content_id(
        &self,
        address: Url,
        parsed: NucleusTarget,
        content_id: u64,
        ...
        for i in 0..part_count {
            ...
            redirects.push(WriteRedirect {
                ...
                scope: RedirectScope {
                    ...
                    expires_at: expires,
                    // `part_headers` carries the LFT connection's own
                    // authentication, which names neither this object nor this
                    // upload and outlives the redirect.
...
pub(crate) struct NucleusContinuation {
    /// The path and branch this continuation was minted for. (`branch` is
    /// always `None` in practice: the only address spelling that yields one is
    /// `?branch&N`, which also yields a checkpoint, and every mutating verb
    /// refuses a checkpoint.) Written to the
    /// encoded form but never read back from it: on the broker's client-driven
    /// route the blob is echoed back by the remote caller, so `continue_write`
    /// re-derives both from the authorized request address.
    /// `skip_deserializing` is what makes a caller-supplied path unreachable
    /// rather than merely unused; both are still serialized so a continuation
    /// minted here stays decodable by a peer replica running an earlier build
...
pub(crate) fn encode_nucleus_continuation(cont: &NucleusContinuation) -> Vec<u8> {
...
/// The Nucleus object/data operations delegated to by
/// `crate::layer::NucleusLayer`. The recovery loop
/// (`ConnectionSet::with_recovery` + `NucleusDriver::refresh`) owns
/// refresh-and-retry-once at the layer.
impl NucleusBackend {
    pub async fn stat(
        &self,
    ...
    pub async fn read(
        ...
        // before the blanket Unsupported refusal below: an inverted
        // range from a buggy caller is best surfaced as a typed error
        // rather than silently falling through into the catch-all
        // message (or panicking a downstream slice).
                    ...
                    if let Some(redirect_url) = result.uri_redirection.as_deref() {
                        ...
                        return Ok(ReadResult::Redirect(ReadRedirect {
                            ...
                            scope: RedirectScope {
                                ...
                                expires_at: expires,
                                // `lft.auth_headers()` are the connection's own
                                // credentials, bound to nothing about this
                                // object.
                                credential: RedirectCredential::Connection,
                            },
    ...
    pub async fn write_stream(
        ...
        // Session gate BEFORE the body is touched: a parked / never-signed-in
        // connection must refuse with `AuthRequired` without draining a single
        // chunk (draining first would buffer an unbounded caller body only to
        // throw it away).
        let _ = self.ops()?;
        // Below LFT threshold: drain and use the buffered create_asset path.
        // At/above threshold: inline streaming PUT requires `Body::Stream`
        // propagation through the LFT client (deferred); draining to
        // `Vec<u8>` would be a memory-DoS half-measure forbidden by the
        // public-gateway streaming policy. Use `write_redirect` instead.
        let lft = self.lft()?;
        // ONE effective inline ceiling drives both the routing decision and
        // the drain cap, so the two can never disagree (a gate on the raw
        // server threshold would happily buffer 64 MiB of an ACCURATELY
        // hinted body only for the clamped cap to reject it). The server
        // hint is bounded on both ends: `should_use_lft` treats
        // `threshold == 0` (the handshake's mapping of an omitted
        // `lft_threshold`) as never-LFT, so 0 means "the default inline
        // cap", not a 0-byte ceiling that would fail every nonempty body;
        // and an excessively large server-supplied threshold must not defeat
        // the LOCAL memory bound, so the cap clamps to a plugin-controlled
        // maximum independent of the hint.
        let byte_cap = lft
            .as_ref()
            .map(|lft| {
                if lft.threshold > 0 {
                    ...
                    DEFAULT_LFT_THRESHOLD_BYTES
                }
            })
            .unwrap_or(DEFAULT_LFT_THRESHOLD_BYTES)
            .min(MAX_BUFFERED_WRITE_BYTES);
        // With an LFT redirect path available, a hint-absent or above-ceiling
        // body routes there IMMEDIATELY — before a single chunk buffers.
        if lft.is_some() && opts.size_hint.is_none_or(|n| n > byte_cap) {
            return Err(Error::new(
                ErrorCode::Unsupported,
                "Nucleus write_stream: streams above the inline buffered-write ceiling require \
                 redirect (use write_redirect with an accurate opts.size_hint); inline streaming \
                 PUT pending Body::Stream propagation through the LFT client",
            ));
        }
        // `size_hint` is advisory, not a memory-safety bound: enforce the
        // ceiling while draining (an understated hint, or a no-LFT
        // connection with no redirect path to gate on, stops at the cap
        // instead of collecting unboundedly), and validate each chunk
        // BEFORE it is copied so the accumulator never transiently exceeds
        // the cap.
        ...
        for chunk in stream {
            ...
            let chunk = chunk?;
            if would_exceed_cap(bytes.len() as u64, chunk.len() as u64, byte_cap) {
                return Err(Error::new(
                    ErrorCode::Unsupported,
                    format!(
                        "Nucleus write_stream: body exceeded the {byte_cap}-byte buffered-write \
                         ceiling (size_hint understated or absent); supply an accurate \
                         opts.size_hint and use write_redirect, or pass Body::Bytes"
                    ),
                ));
            }
    ...
    pub async fn continue_write(
        ...
        // Derive the asset from the authorized request address — the same
        // expression `write_redirect` used at mint — rather than reading the
        // path back out of a blob the remote caller echoes.
        let parsed = self.target(&target)?;
        // `write_redirect` refuses a `?checkpoint=N` address, and
        // `finalize_with_content_id` would silently commit to the branch head,
        // so the derived target has to be refused here for the same reason.
        Self::reject_checkpoint(&parsed, "continue_write")?;
        ...
        let result = race_cancel(cancel.as_ref(), {
            let parsed = parsed.clone();
            let opts = opts.clone();
            let address = address.clone();
            async move {
    ...
    pub async fn list(
        ...
        race_cancel(cancel.as_ref(), {
            ...
            async move {
                let ops = self.ops()?;
                let path = directory_path(&parsed.path);
                let branches = parsed.branch.clone().map(|b| vec![b]);
                let responses = ops.list2(path.clone(), branches, None, None).await?;
                ...
                'outer: for response in responses {
                    for entry in response.entries.into_iter().flatten() {
                        let Some(item) =
                            list_entry_to_item(&prefix_address, &path, entry, recursive)?
                        else {
                            continue;
                        };
                        items.push(item);
    ...
    pub async fn watch_directory(
        ...
        let watched_prefix = directory_path(&parsed.path);
        let handle = race_cancel(cancel.as_ref(), {
            let parsed = parsed.clone();
            async move {
                let ops = self.ops()?;
                let path = PathAtBranch {
                    path: directory_path(&parsed.path),
...
/// Plugin-controlled ceiling on how much of a streamed body the buffered
/// inline-write path will ever hold in memory, independent of the
/// server-advertised LFT threshold — a hostile or misconfigured server hint
/// must not defeat the local memory bound.
pub(crate) const MAX_BUFFERED_WRITE_BYTES: u64 = 4 * DEFAULT_LFT_THRESHOLD_BYTES;

/// Whether appending `chunk_len` bytes to a `current_len`-byte accumulator
/// would push it past `cap` — evaluated BEFORE the copy so the accumulator is
/// never transiently grown beyond the cap. `saturating_add` keeps the check
/// correct even for a pathologically large `chunk_len` (a `current_len` near
/// `u64::MAX` must report "exceeds" rather than wrap to a small sum and admit
/// the chunk). Extracted so the overflow-safe boundary is unit-testable.
pub(crate) fn would_exceed_cap(current_len: u64, chunk_len: u64, cap: u64) -> bool {
