...
//!    decision: the broker daemon's `write_redirect` delegates to the
//!    backend-emitted write-redirect protocol, which accepts unknown
//!    size (`Option<u64>`) and lets the backend presign. The plugin
//!    must therefore *forward* `size_hint = None`, not refuse it —
...
use ovstorage_plugin::{
    ...
    InteractiveAuthCapability, ListOptions, ListPage, ListVersionsOptions, ObjectInfo, ObjectKind,
    ReadOptions, ReadResult, RedirectResultBatch, RenameOptions, ResolvedTarget, Result,
    StatOptions, UpdateMetadataOptions, Url, WatchDirectoryOptions, WriteOptions,
    WriteRedirectBatch, WriteResult, WriteStep, address,
};
...
async fn write_redirect_forwards_none_size_hint_faithfully() {
    ...
    // doc-comment): the broker daemon forwards unknown-size writes to the
    // backend-emitted write-redirect protocol; refusing here would deny a
    // path the wire fully supports.
    let recorder = Arc::new(RecordingTransport::default());
    let backend = backend_with(recorder.clone());
