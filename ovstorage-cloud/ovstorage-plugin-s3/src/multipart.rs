...
/// Tag for refusing foreign-plugin continuation payloads.
...
pub struct MultipartContinuation {
    pub tag: String,
    /// The object key. Written to the encoded form but never read back from
    /// it: on the broker's client-driven route the blob is echoed back by the
    /// remote caller, so `continue_write` derives the key from the authorized
    /// request address. `skip_deserializing` is what makes a caller-supplied
    /// key unreachable rather than merely unused; it is still serialized so a
...
mod tests {
    ...
    /// The key still rides the encoded blob so a peer replica on an earlier
    /// build can decode a continuation minted here, but a key the caller puts
    /// there is unreachable: `decode` never populates the field.
    #[test]
    fn continuation_emits_the_key_but_never_reads_it_back() {
        let c = MultipartContinuation::new(
            "authorized/key".into(),
            "upload-id".into(),
            None,
        ...
        // Decode into a mirror of the shape an older build requires, rather than
        // substring-matching the value: a rename, a retype or a nesting change
        // would keep the string present while breaking the peer that has to
        // parse it, and that peer is the whole reason the field is still here.
        // Every field the pre-derivation decoder required, so a rename or a
        // retype of any of them fails here rather than at an older peer. Serde
        // ignores unknown fields but errors on a missing one, which is exactly
        // the failure this is standing in for.
        #[derive(serde::Deserialize)]
        #[allow(dead_code)]
        struct LegacyMultipartContinuation {
            tag: String,
            key: String,
            upload_id: String,
            user_metadata: Option<Vec<(String, String)>>,
        ...
        let legacy: LegacyMultipartContinuation = serde_json::from_slice(&c.encode()).unwrap();
        assert_eq!(legacy.key, "authorized/key");
        assert_eq!(legacy.tag, CONTINUATION_TAG);
        assert_eq!(legacy.upload_id, "upload-id");
        assert_eq!(legacy.total_parts, 1);

        let forged = br#"{"tag":"ovstorage-plugin-s3:multipart:1","key":"victim/key","upload_id":"u","user_metadata":null,"if_match":null,"no_overwrite":false,"parts":[],"total_parts":0}"#;
        let decoded = MultipartContinuation::decode(forged).unwrap();
        assert_eq!(decoded.key, "");
        assert_eq!(decoded.upload_id, "u");
    }
    ...
    /// Headline case: 10 GiB / 32 MiB = 320 parts of
    /// ~32 MiB each, all within S3's [5 MiB, 5 GiB] window. A
    /// maximum-size split would instead yield 2 parts of 5 GiB.
    #[test]
