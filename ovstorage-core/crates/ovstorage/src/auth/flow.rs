...
async fn run_pkce(
    ...
    let stream = async_stream::try_stream! {
        ...
        let credentials = credentials_from_token(&token);
...
async fn run_device(
    ...
    let stream = async_stream::try_stream! {
        ...
        let expires_at = SystemTime::now() + Duration::from_secs(device.expires_in);
...
async fn exchange_code(
    ...
    let body = response.bytes().await.map_err(|err| {
        Error::new(
            ErrorCode::Transient,
            format!("OAuthFlow::pkce: token body read failed: {err}"),
        )
    })?;
...
async fn request_device_code(
    ...
    let body = response.bytes().await.map_err(|err| {
        Error::new(
            ErrorCode::Transient,
            format!("OAuthFlow::device: device-auth body read failed: {err}"),
        )
    })?;
...
async fn poll_device_token(
    ...
    let body = response.bytes().await.map_err(|err| {
        DevicePollError::Terminal(Error::new(
            ErrorCode::Transient,
            format!("OAuthFlow::device: poll body read failed: {err}"),
        ))
    })?;
    ...
    if let Ok(parsed) = serde_json::from_slice::<TokenError>(&body) {
        return match parsed.error.as_str() {
            ...
            other => Err(DevicePollError::Terminal(Error::new(
                ErrorCode::AuthRequired,
                format!("OAuthFlow::device: token endpoint returned '{other}'"),
            ))),
        };
    }
...
fn credentials_from_token(token: &TokenResponse) -> ovstorage_plugin::SecretBundle {
...
fn build_succeeded_connection(
    ...
    let expires_at = token.expires_in.map(|secs| now + Duration::from_secs(secs));
    Connection {
        id: connection_id
            .cloned()
                    ...
                    tokio::select! {
                        ...
                        accept = listener.accept() => {
                            if let Ok((sock, _)) = accept {
                                ...
                                tokio::spawn(handle_idp_request(sock, attempts, token));
                            }
                        }
                    }
                }
