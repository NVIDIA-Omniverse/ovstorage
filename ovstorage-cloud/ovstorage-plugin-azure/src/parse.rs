...
pub(crate) fn list_xml_to_object_infos(
    ...
    for blob in &parsed.items {
        let Ok(blob_address) = address::join_relative(address_root, &blob.name) else {
            tracing::warn!(
                target: "ovstorage.azure.parse",
                plugin = "azure",
                key = %blob.name,
                "azure: blob name is not addressable as a URI path; omitted from listing",
            );
            continue;
        };
    ...
    for prefix in &parsed.prefixes {
        let Ok(address) = address::join_relative(address_root, prefix) else {
            tracing::warn!(
                target: "ovstorage.azure.parse",
                plugin = "azure",
                key = %prefix,
                "azure: prefix is not addressable as a URI path; omitted from listing",
            );
            continue;
        };
...
pub(crate) fn dfs_paths_to_object_infos(
    ...
    parsed
        ...
        .iter()
        .filter_map(|path| {
            let Ok(address) = address::join_relative(address_root, &path.name) else {
                tracing::warn!(
                    target: "ovstorage.azure.parse",
                    plugin = "azure",
                    key = %path.name,
                    "azure: path name is not addressable as a URI path; omitted from listing",
                );
                return None;
            };
...
pub(crate) fn blob_to_version_item(
    ...
    let Ok(blob_address) = address::join_relative(address_root, &blob.name) else {
        // A version listing is a listing: omit the entry rather than failing
        // the page and hiding every other version with it. `None` is the skip
        // channel this function already has.
        tracing::warn!(
            target: "ovstorage.azure.parse",
            plugin = "azure",
            key = %blob.name,
            "azure: blob name is not addressable as a URI path; omitted from version listing",
        );
        return Ok(None);
    };
...
<EnumerationResults ServiceEndpoint="https://acct.blob.core.windows.net/" ContainerName="container">
  ...
  "paths": [
    ...
    /// A version listing omits an unaddressable blob instead of failing.
    ///
    /// A blob whose name contains `//` cannot be spelled as a URI path — the
    /// empty segment collapses, so the address would name a different blob.
    /// Propagating that would fail the whole page and hide every other version
    /// with it. Invisible beats mis-addressed, and it beats unavailable.
    #[test]
    fn a_version_whose_name_has_no_address_is_omitted_not_propagated() {
        let blob = BlobListEntry {
            name: "a//b.bin".into(),
            etag: Some("0x1".into()),
            size: Some(7),
        ...
        let root = Url::parse("azure://acct/container/").unwrap();
        assert!(
            blob_to_version_item(&blob, &root)
                .expect("an unaddressable name must not fail the page")
                .is_none(),
            "an unaddressable name must be omitted from the listing"
        );
    }
