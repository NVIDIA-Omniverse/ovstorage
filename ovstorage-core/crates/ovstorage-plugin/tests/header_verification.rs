...
//!
//! - `c_example_compiles` — invokes `cc` on
//!   `examples/plugin-c/example_plugin.c` against the generated header.
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is `crates/ovstorage-plugin`; the workspace
...
fn header_naming_conventions() {
    ...
    // Sanity check: ABI version constant is the one item exempted
    // from the project prefix (via `[export.rename]`).
    assert!(
    ...
    for line in header.lines() {
        ...
        if let Some(name) = parse_typedef_name(trimmed) {
            if name == "OVSTORAGE_PLUGIN_ABI_VERSION" {
                continue;
            }
            if !name.starts_with("OvStoragePlugin_") {
...
fn c_example_compiles() {
    ...
    let probe = match probe {
        Ok(out) => out,
        Err(_) => {
            eprintln!("skipping c_example_compiles: `cc` not found on PATH");
            return;
        }
    };
    if !probe.status.success() {
        eprintln!("skipping c_example_compiles: `cc --version` failed");
        return;
    }

    let workspace = workspace_root();
    let example_c = workspace
        .join("examples")
        .join("plugin-c")
        .join("example_plugin.c");
    assert!(
        example_c.is_file(),
        "expected example C plugin at {}",
        example_c.display(),
    );

    let include_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("include");
    let temp_dir = std::env::temp_dir();
    let object = temp_dir.join("ovstorage_plugin_header_verification.o");
    ...
    if !output.status.success() {
        ...
        panic!(
            "cc failed to compile examples/plugin-c/example_plugin.c against the \
             generated header.\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
        );
    }
    let _ = std::fs::remove_file(&object);
