...
use ovstorage::{
    ...
    ErrorCode, ListOptions, ObjectKind, Request, Stack, Url, WatchDirectoryCursor,
    WatchDirectoryOptions, WatchDirectoryRequest,
};
...
pub(crate) async fn delete_directory(
    ...
    let entries = enumerate_recursive(stack, &resolved, cancel).await?;
    ...
    execute_delete_plan(stack, resolved, entries, cancel).await
}
...
async fn enumerate_recursive(
    ...
    let recursive_result =
        collect_list_page(stack, root, true, true, &mut out, &mut seen, cancel).await;
...
fn push_delete_plan_entry(
    ...
    if out.len() >= 100_000 {
        // `Internal`, not `ResourceExhausted`: this is a fixed local bound on
        // what one command may hold, so it is reached identically on every
        // attempt and a retry only pays the enumeration again. See the
        // error-code table in `docs/public/plugin-storage/CONFORMANCE.md`.
        return Err(Error::new(
            ErrorCode::Internal,
...
async fn execute_delete_plan(
    ...
    for address in objects {
        ...
        stack
            .delete(address, DeleteOptions::default(), Some(cancel.clone()))
            .await?;
    }
    ...
    for address in directories {
        ...
        stack
            .delete_directory(address, DeleteDirectoryOptions, Some(cancel.clone()))
            .await?;
    }

    stack
        .delete_directory(root, DeleteDirectoryOptions, Some(cancel.clone()))
        .await
}
...
pub(crate) async fn watch_directory(
    ...
    let since = since
        ...
        .transpose()?;
    // `watch_directory` is a `Layer` primitive with no `LayerExt` verb; UFCS
    // avoids a trait-method collision with `LayerExt`'s data-plane verbs.
    let mut stream = ovstorage::Layer::watch_directory(
        state.stack.as_ref(),
        Request::new(WatchDirectoryRequest {
            prefix: resolve_address(prefix, state.pwd.as_ref())?,
