...
//! Scenario runner harness and report format. Verifies recorded
//! vtable-slot call sequences against [`crate::ScenarioRegistry`]
//! entries and aggregates per-scenario outcomes for snapshot diffing.
...
impl<'a> ScenarioRunner<'a> {
    ...
    pub fn verify_with_failure(
        ...
        // When the contract names a failing method and an error WAS
        // observed, methods after the failing one are legitimately never
        // invoked — verify the sequence only up to (and including) it.
        let expected = match (&scenario.failure_contract, &observed_error) {
            (FailureContract::Errors { method, .. }, Some(_))
            | (FailureContract::SuccessOrRefusal { method, .. }, Some(_)) => {
                truncate_at_failing_method(scenario.expected_calls, method)
            }
            _ => scenario.expected_calls,
        };
        if let Err(reason) = check_call_sequence(expected, &recorded) {
            return ScenarioReport::failed(scenario, reason, recorded);
        }
        match (&scenario.failure_contract, observed_error) {
            (FailureContract::Success, None) | (FailureContract::SuccessOrRefusal { .. }, None) => {
            ...
            (
                FailureContract::Errors { method, code }
                | FailureContract::SuccessOrRefusal { method, code },
                Some((m, c)),
            ) => {
                if &m == method && c == *code {
                    ScenarioReport::passed(scenario, recorded)
...
/// The expectation slice for a scenario whose contract-named `method`
/// refused: everything up to and including the LAST expected occurrence
/// of `method` ("later methods aren't invoked"). Falls back to the full
/// slice when `method` never appears in `expected_calls`.
fn truncate_at_failing_method<'a>(
    expected: &'a [ExpectedCall],
    method: &str,
) -> &'a [ExpectedCall] {
    match expected.iter().rposition(|e| e.method == method) {
        Some(i) => &expected[..=i],
        None => expected,
    }
}
...
mod tests {
    ...
    fn copy_to_self_calls(with_read: bool) -> Vec<ObservedCall> {
        let url = Url::parse("test://demo/self.txt").unwrap();
        let mut calls = vec![
            ObservedCall::Write {
                target: url.clone(),
                byte_len: 14,
        ...
        if with_read {
            calls.push(ObservedCall::Read { target: url });
        }
        calls
    }

    /// `SuccessOrRefusal`, success half: the full write→copy→read sequence
    /// with no error passes.
    #[test]
    fn success_or_refusal_passes_on_full_success_sequence() {
        let registry = fixture_registry();
        let runner = ScenarioRunner::new(&registry);
        let report = runner.verify_with_failure(
            "copy-to-self-preserves-content",
            copy_to_self_calls(true),
            None,
        );
        assert!(matches!(report.outcome, ScenarioOutcome::Passed));
    }

    /// `SuccessOrRefusal`, refusal half: the contract-named method refuses
    /// with the contract code, the post-refusal `read` never happens, and
    /// the truncated expectation still passes.
    #[test]
    fn success_or_refusal_passes_on_typed_refusal_without_later_calls() {
        let registry = fixture_registry();
        let runner = ScenarioRunner::new(&registry);
        let report = runner.verify_with_failure(
            "copy-to-self-preserves-content",
            copy_to_self_calls(false),
            Some(("copy".into(), ErrorCode::Conflict)),
        );
        assert!(matches!(report.outcome, ScenarioOutcome::Passed));
    }

    /// `SuccessOrRefusal` still rejects the wrong refusal code and a
    /// truncated sequence WITHOUT a refusal (silent early stop).
    #[test]
    fn success_or_refusal_rejects_wrong_code_and_silent_truncation() {
        let registry = fixture_registry();
        let runner = ScenarioRunner::new(&registry);
        let report = runner.verify_with_failure(
            "copy-to-self-preserves-content",
            copy_to_self_calls(false),
            Some(("copy".into(), ErrorCode::Internal)),
        );
        assert!(matches!(report.outcome, ScenarioOutcome::Failed { .. }));
        let report = runner.verify_with_failure(
            "copy-to-self-preserves-content",
        ...
        assert!(
            matches!(report.outcome, ScenarioOutcome::Failed { .. }),
            "a success outcome with the read missing must fail the sequence check"
        );
    }
