...
//! Structured vtable-slot call log; siblings the per-method counter
//! map in the internal `store` module.

use std::sync::{Arc, Mutex};

use ovstorage_plugin::{ResolvedTarget, Url};

/// One observed operational call. One variant per recorded slot; names match
/// the `OvStorage_LayerVTable` slots. `WatchAddressRoots` is recorded by
/// the operation names observed through the native `Layer` surface.
...
impl ObservedCall {
    /// Bare slot method name; matches [`crate::scenarios::ExpectedCall::method`].
    pub fn method_name(&self) -> &'static str {
        match self {
            ...
            ObservedCall::ListAddressRoots => "list_address_roots",
