...
fn shared_single_writer_rejects_second_writer_at_open() {
    ...
    assert_eq!(err.code(), ovstorage_layer::ErrorCode::CacheLockContention);
}
...
fn lease_pinning_survives_eviction_pressure_in_one_process() {
    ...
    drop(lease);
    // One more put pushes us over budget again; with the lease dropped the
    // row is unpinned and may be evicted.
