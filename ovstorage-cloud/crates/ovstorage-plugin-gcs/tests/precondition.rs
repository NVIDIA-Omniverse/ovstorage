...
use ovstorage_plugin::{
    ...
    ErrorCode, ReadOptions, ReadResult, ResolvedTarget, Result, SecretBundle, SecretBytes,
    SecretValue, WriteOptions, address,
};
...
async fn anonymous_backend(endpoint: &str, bucket: &str) -> Arc<dyn Backend> {
    ...
    GcsBackendFactory::new()
        ...
        .await
        .expect("factory instantiate")
        .backend
}

async fn service_account_backend(endpoint: &str, bucket: &str) -> Arc<dyn Backend> {
    let mut config = HashMap::new();
    config.insert("bucket".into(), ConfigValue::String(bucket.into()));
    config.insert("endpoint".into(), ConfigValue::String(endpoint.into()));
    let request = ConnectionRequest {
