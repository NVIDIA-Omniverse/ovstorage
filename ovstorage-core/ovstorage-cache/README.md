...
>
> **What's implemented.** Versioned `migrations` framework with `schema_version` table; `cache_entries` table with `cas_key` PK; `CURRENT_SCHEMA_VERSION = 2` exposed at the crate root. The byte cache implementation now lives behind `ByteCache`; `Cache` remains a compatibility alias for existing callers until the host cutover. The `HerdKey { Current, Guarded, Exact }` enum + `ByteCache::with_typed_herd_lock`; the legacy `with_herd_lock(&str)` keeps working unchanged. Different flavors hash into disjoint lock spaces. `lease::{CacheProcess, Lease}` (RAII drop semantics, cleanup-closure pattern), `fs_probe::{fs_kind, FsKind}` + `OVSTORAGE_ALLOW_NETWORK_FS` env override, `coordination::{CacheCoordination, acquire_writer_rendezvous}`, `observer::{Observer, LookupOutcome, FillOutcome, EvictionReason, NoopObserver}` — all wired into `ByteCache::open` / `put` / `get_entry` / `evict_to_limit`. `CacheOptions` carries `coordination` + `observer` fields. The in-memory `MetadataCache` implementation and its TTL, budget, option-hash, and invalidation tests are also ported into this crate; `ovstorage` still owns the active host wiring until the factory cutover. The on-disk process sentinel at `<state_root>/processes/<pid>.lock` is owned by `CacheProcess` itself; the on-disk file is unlocked and unlinked only when the last `Arc<CacheProcess>` drops, so a live `Lease` or sibling `ByteCache` handle keeps the sentinel held across the originating cache's drop. `ReadOnly` coordination is enforced through a single `ensure_writable` chokepoint that gates `put`, `put_with_existing_key_lock`, `remove_index`, and `remove_prefix`. `fs_kind` consults `statfs(2).f_type` on Linux against the NFS/CIFS/SMB/FUSE/Lustre/Ceph/GFS2/AFS/OCFS2 magic-number table; non-Linux targets fall back to the UNC string heuristic. The `ByteCache::lease(cas_key)` API mints leases that bump `cache_entries.lease_count` and pin rows against eviction; `lease()` verifies the backing CAS file exists before minting so a stale `cache_entries` row never produces a lease pointing at nothing.
>
...
>
> **Remaining as (spec).** The `auth.sqlite` separation *as this crate's own second database* — the file itself ships, but `ovstorage` owns it and this crate neither creates nor locates it — the full `current_index` / `exact_index` / `guarded_index` schema split, the 11 metric families' field-level wiring (only the subset emitted by the current `Observer` calls is implemented), the `staging` / `policy_snapshots` / `routes` tables, and the dedicated `herd/<kind>/<key>.lock` / `gc.lock` directory trees remain spec-only.

## Purpose (partial)

`ovstorage-cache` is a library, not a daemon: it is compiled into both client processes (via `ovstorage`) and the broker (`ovstorage-broker`) directly, and there is no cache RPC. It jointly owns `state_root` (durable state: SQLite WAL index, leases, staging metadata, policy snapshots — but not the auth database, which `ovstorage` owns and keeps under a directory of its own) and `cache_root` (opaque content-addressed object bytes), and provides the cross-process coordination primitives that let many processes — laptops, CI jobs, containers — share that on-disk state without a coordination daemon.

Concretely the crate owns:

- CAS file lifecycle keyed on `content_digest`.
- SQLite WAL index for `current_index`, `exact_index`, `guarded_index`, `cache_entries`, `leases`, `staging`, `policy_snapshots`, `routes`. `auth.sqlite` is *not* one of them — it is `ovstorage`'s, under a directory of its own.
...
- **Auth state:** serialize OAuth refresh (cross-process) and persist credentials in `auth.sqlite`; the OAuth protocol itself remains in `ovstorage`. **(implemented)** — `ovstorage::auth` owns `auth.sqlite` (`refresh_records` for cross-process refresh coordination + `credential_records` for persisted credential metadata + `secrets` for the credential bytes + `connection_address_roots` for cached per-connection address roots). Two writers reach it: `ovstorage::auth::OAuthCredentialProvider` calls `SecretStore::put_many` directly and records the metadata row, and a loaded plugin persists its own refresh token through the host's `secret_put` callback, which the Rust host backs with the same `SecretStore`. Where the auth directory resolves is `ovstorage`'s decision and not this crate's. See "Secret handling on disk" below.
...
<state_root>/
  ...
  # spec: auth/<backend>.lock         # flock'd credential-refresh mutex (one per backend); the
                                      #  shipped equivalent is <auth_dir>/locks/auth/<sha256>.lock
                                      #  below, keyed per (backend_kind, connection_id)
...
<auth_dir>/                           # a third directory; not derived from state_root or cache_root
  auth.sqlite                         # owned by ovstorage::auth::AuthRefreshLock (refresh_records,
                                      #  credential_records, secrets, connection_address_roots)
  auth.sqlite-wal
  auth.sqlite-shm
  locks/auth/<sha256>.lock            # flock'd refresh mutex, one per (backend_kind, connection_id)
```

`<auth_dir>` is not one of this crate's roots unless an operator points it at
one — `OVSTORAGE_AUTH_DIR` and `ovstorage::init_auth_substrate` both take an
arbitrary path. What this crate guarantees is narrower: it never derives the
auth directory from `state_root` or `cache_root`, and no call site in this
workspace does either. Which directory a given process actually used is
`ovstorage`'s to answer — the substrate there resolves it, and `ovstorage-cache`
has no code on that path and cannot observe it.
...
Override via config or `OVSTORAGE_STATE_ROOT` / `OVSTORAGE_CACHE_ROOT`; `<auth_dir>` has its own, `OVSTORAGE_AUTH_DIR`, and neither of these covers it. Two roots = two independent lifecycle policies for the two this crate manages; operators can delete `cache_root` while inactive to free disk without invalidating durable state. Active wipes may cause misses or `CacheCorrupt` but must not corrupt `state_root`.
...
- **Credential refresh**: `flock` + SQLite `BEGIN EXCLUSIVE` against `auth/<backend>.lock` + the `refresh_records` row in `auth.sqlite`. In the shipped code that lock and that database are `ovstorage`'s, under `<auth_dir>`, not this crate's under `state_root`. In the target shape here, the cache crate owns the lock file, the SQLite serialization, and the on-disk token-storage shape (see [SQLite schema](#sqlite-schema-sketch-spec--current-schema-is-much-smaller)); [ovstorage](../ovstorage/README.md) owns the credential flows (OAuth, S3 STS, Azure SAS, etc.) that drive the refresh. The same machinery serializes any backend whose credential is durable and needs single-writer refresh — not just OAuth.
- **GC**: `flock` on `gc.lock`.
...
> ```
>
> The shape also has `cache_entries(cas_key PK, size, pin_count, lease_count, verified_at)` (used by `Cache::lease`, eviction, and recovery), `entries(resolved_target PK, cas_key, size, updated_unix_ms, last_access_unix_ms)`, `process_leases(pid, started_unix_ms, state_root, PK (pid, started_unix_ms))`, and `schema_version(rowid PK, version)`. Every other table below (`current_index`, `exact_index`, `guarded_index`, `processes`, `leases`, `staging`, `policy_snapshots`, `routes`) and every secondary index listed are **(spec)**. Auth-DB tables are owned by `ovstorage::auth`, which creates four: `refresh_records`, `credential_records`, `secrets`, `connection_address_roots`.
...
- `<auth_dir>/auth.sqlite` — durable secret/token storage and cross-process refresh coordination for any backend whose credential needs single-writer refresh (OAuth refresh tokens, S3 STS session tokens, Azure SAS handles, etc.). `synchronous = FULL`. `<auth_dir>` is `ovstorage`'s, not derived from a cache root; see [Two roots, one library](#two-roots-one-library-partial--file-layout-differs-from-spec).
...
- `refresh_records(backend_kind, connection_id, refreshed_unix_ms, expires_at_unix_ms, PK(backend_kind, connection_id))` — cross-process refresh coordination. Holds the timestamp of the last successful refresh and its expiry so concurrent processes can decide whether to skip a refresh round trip. Generic over the credential kind: OAuth, S3 STS, Azure SAS, or any other refresh-shaped credential funnels through here.
- `credential_records(backend_id, principal, cred_epoch, inserted_unix_ms, expires_at_unix_ms, secret_handle, source_name, PK(backend_id, principal))` — the durable index for persisted credentials. The row carries `secret_handle`, never the secret bytes; the bytes are a row in `secrets` under that handle. The table is credential-kind agnostic deliberately: the same row plumbs S3 STS session keys, Azure SAS strings, and bearer-style tokens just as well as OAuth refresh tokens. `cred_epoch` lets a callback advertise that already-issued credentials are now suspect (rotation, manual revocation) without dropping the row. It replaced a `secret_tokens` table whose handle named an OS-keyring entry; an `auth.sqlite` written by an older release still carries that table, unread.
- `connection_address_roots(identity_key PK, backend_kind, display_name, addresses_json, cached_unix_ms)` — the address roots last seen for a connection, cached as JSON so a host can present them before the backend is reachable. Carries no credential material. The table is created and its `load_address_roots` / `store_address_roots` / `delete_address_roots` accessors exist, but nothing in the workspace calls them outside this module's own round-trip test: the intent has shipped as a seam, not as a live cache.
- `secrets(backend_kind, connection_id, field, secret, updated_unix_ms, PK(backend_kind, connection_id, field))` — the credential bytes. Keyed on the plugin ABI's own composite key, so the `secret_get` / `secret_put` / `secret_delete` host callbacks map onto rows one-to-one and the host's own `OAuthCredentialProvider` addresses the same table. A `secret_blobs(handle, …)` table was specified here previously and is **withdrawn**: it was keyed on a handle, which only the host pipeline mints, so a plugin reaching the store across the ABI would have had nowhere to write.
...
Both DBs share the same crash-recovery sweep (see [Crash recovery](#crash-recovery-spec--only-staging-cleanup-is-implemented)). They do **not** share a root: `index.sqlite` is under `state_root`, `auth.sqlite` is under `<auth_dir>` (see [Two roots, one library](#two-roots-one-library-partial--file-layout-differs-from-spec)), so a backup that covers `state_root` covers only the first of them.
...
- `auth.sqlite` contains auth state only — today `refresh_records`, `credential_records`, `secrets`, and `connection_address_roots`. No cache, lease, or policy table may be added to it; otherwise the durability split stops being meaningful.
...
5. **Index repair.** Recompute denormalized `lease_count` for touched CAS keys, delete index rows that reference missing entries, and clear guarded rows for targets whose current identity does not match.
...
`ovstorage-cache` emits metrics and tracing through the host's observability sink; it does not open sockets, start exporters, or own a runtime. Direct Stack hosts inherit the opt-in client metrics surface from [ovstorage](../ovstorage/README.md); brokers inherit their own always-on broker metrics surface.
...
- Manual deletion of `cache_root` is supported while no process is actively using that root. Deletion during active use can produce misses or `CacheCorrupt`, but must not corrupt `state_root`. A cache wipe is not the operation for clearing credentials: nothing here puts the auth database under either root — though an operator who pointed `OVSTORAGE_AUTH_DIR` at one has put it in the blast radius themselves.
- Manual deletion of `state_root` discards the cache index and the coordination state; it is a supported recovery action but not a cache-clearing operation. Clearing stored credentials is a separate operation on separate storage, which this crate neither owns nor locates; deleting `state_root` is not it.
...
**Secret handling on disk.** Credential bytes live in `auth.sqlite`, in a `secrets` table keyed `(backend_kind, connection_id, field)`. `credential_records` holds metadata beside them — expiry, `cred_epoch`, and a `secret_handle` naming the bytes — and never the bytes themselves. Both are in the same database and the same WAL, so a credential and its metadata commit together.

They are stored **unencrypted**, and this repository makes no encryption-at-rest claim. On Linux this is a change: an earlier release put the bytes in the kernel keyutils store, where they were held in kernel memory and a refresh token never reached disk. That was traded deliberately. Keyutils was a real secret store on only two of the four host substrates this workspace ships, its per-user quota (`maxbytes=20000`, shared across the whole login session with Kerberos, NFS and containers) capped a user at roughly five to nine connections, and its durability was a runtime property the process could not interrogate — where the kernel offered no persistent keyring, commonly in exactly the containers and CI runners that most wanted one, a credential did not survive the session. Encrypting the database under a key held in that same store was considered and rejected: on Linux the key would have been the volatile part, so after a reboot the database would be present and unreadable.

**Who may read the bytes is decided by file permissions.** The auth directory is created `0700` and `auth.sqlite` `0600`, with an owner-only DACL on Windows, and both are corrected on every open rather than only on the one that creates them — an operator upgrading from a release that created the directory at the default umask gets it tightened before any credential byte is written into it. That permission is the whole of the separation: it stops another OS user on the box reading the credentials, and it stops them landing in a backup or disk image readable by someone else. It does not stop a process running as the same user, and nothing here claims to. Cache CAS bytes never contain credential material — provider replies are filtered against the response-parsing rules in [ovstorage-plugin](../ovstorage-plugin/README.md) before any byte is staged.

**`cache_root` content.** Cache files are byte-identical copies of objects fetched on the caller's behalf; whoever can read `cache_root` can read those objects. Operators rely on filesystem permissions on `cache_root` for that boundary — `0700` is the recommendation, not a mode the crate applies; it creates these directories with the process umask and sets nothing. The cache crate does not encrypt at rest.

**`state_root` content.** `state_root` may contain policy snapshots, route metadata, and the herd-collapse / lease bookkeeping. None of that is sensitive on its own, but it does fingerprint what the caller has been reading; operators with that concern protect `state_root` with the same filesystem-ownership model as `cache_root`.

**What the cache roots do not cover.** The credential rows described above are in `auth.sqlite`, which this crate does not create, locate, or protect. Unless an operator has deliberately pointed the auth directory inside one of them, permissions applied to `state_root` and `cache_root` do not reach it, and an operator who wants that file protected has to do it separately.
...
- Process-sentinel sharing: multiple cache handles in one process and one canonical `state_root` share one sentinel.
- `LocalDelegate` lifetime: dropping the originating Stack while a `LocalDelegate` survives keeps the sentinel live and prevents eviction.
...
- The schema uses versioned migrations from the start. `index.sqlite` and `auth.sqlite` each carry a `schema_version` row plus upgrade tests from every checked-in fixture version. **Current:** only `index.sqlite` has `schema_version`; `auth.sqlite` is created by `CREATE TABLE IF NOT EXISTS` with no version row.
...
- `state_root` and `cache_root` independently configurable; `cache = None` is supported and skips the cache layer entirely (state DB still active for connection metadata; auth state is in a separate database under its own directory and is unaffected either way).
...
- **A secret store with no SQLite coordination.** A store holds refresh-token bytes; it is not a cross-process refresh protocol. The SQLite row plus the per-backend advisory lock (shipped as `<auth_dir>/locks/auth/<sha256>.lock`) gives every process the same refresh metadata, timestamps, and serialization point, and the token bytes are a row in the same database.
...
- `auth.sqlite` schema: `refresh_records(backend_kind, connection_id, refreshed_unix_ms, expires_at_unix_ms)`, `credential_records(backend_id, principal, cred_epoch, inserted_unix_ms, expires_at_unix_ms, secret_handle, source_name)`, `secrets(backend_kind, connection_id, field, secret, updated_unix_ms)`. Generic over the credential kind — OAuth, S3 STS, Azure SAS, etc. — so a single durable shape covers every refresh-shaped credential the workspace plumbs. **Current:** all four are created, `secrets` and `credential_records` by `ovstorage::auth`, and the database is opened `0600` under a `0700` directory.
- `BEGIN EXCLUSIVE` around every refresh: fetch the current row, perform the IdP refresh request, write the new secret handle/timestamps, commit. The exclusive transaction serializes refresh attempts within a single process; cross-process serialization uses `flock(<state_root>/auth/<backend>.lock, LOCK_EX)` held across the entire refresh. **Current:** the per-`(backend_kind, connection_id)` advisory file lock ships, but under `<auth_dir>/locks/auth/<sha256>.lock` rather than the `<state_root>` path above, and the SQLite write does **not** wrap in `BEGIN EXCLUSIVE` — it relies on a single `INSERT ... ON CONFLICT DO UPDATE`.
- `synchronous = FULL`, `journal_mode = WAL`, `busy_timeout = 30000` (refresh can wait up to 30s for the lock).
- The refreshed token bytes are written before the metadata row is committed, and the commit publishes the new durable handle. Both are rows in one database now, so a single transaction can carry them together; `SecretStore::put_many` gives that guarantee across the fields of one connection. If SQLite commit succeeds, every process must be able to resolve the committed handle.
- There is no `ovstorage`-level backend selector, on either path. Both live writers — `OAuthCredentialProvider` and the host's `secret_put` callback — go to `SqliteSecretStore`, and nothing in this repository's configuration can redirect them; `SecretStore` is a trait, so a different backing is a code change rather than a config one. Nothing reports which store is in use, so `state-status` has nothing to represent. A database-resident backend and an encrypted-file backend both remain **(spec)**; each would need a selector, a startup diagnostic, and a field in `state-status` output to report the choice — none of which exist.
