...
//! and documented as gaps in `docs/public/plugin-storage/plugin-azure.md`; they would each
...
pub(crate) enum AuthSource {
    /// No credentials supplied. Requests go out unsigned and the service
    /// decides: what succeeds is what the container's public-access level
    /// allows — `blob` permits reading a blob, `container` also permits
    /// enumerating one. There are no "signed operations" on this source to
    /// fail, and a refusal arrives as whatever Azure sends, which for an
    /// enumeration a `blob`-level container will not serve is a `404` rather
    /// than a challenge. What the plugin itself refuses is the delegated-write
    /// pair — `write_redirect`, which has no SAS to delegate with, and
    /// `continue_write`, which only runs after one and is gated by the same
...
pub struct AzureAuth {
    ...
    /// to [`AzureAuth::bearer_token`] because they own their own connection pool.
    http: Client,
    ...
    /// Latched when the IdP refuses a grant for this credential, and cleared
    /// only by a later successful one.
    ///
    /// The promotion veto is otherwise bounded by an operation's lifetime: a
    /// witness requires an acceptance of its own and no refusal on the
    /// connection while it ran. A refusal the background refresh loop meets
    /// escapes that regardless of how it overlaps in time, because the refresh
    /// talks to the IdP rather than to storage and so advances no refusal
    /// epoch; and it leaves the cached bearer in place, which the service goes
    /// on accepting until that bearer expires. So an operation in that window
    /// genuinely does see an acceptance and no refusal, and would promote a
    /// connection whose renewal credential is already dead — which nothing can
    /// undo, because Azure has no `refresh` and its credentials are frozen at
    /// registration.
    ///
    /// A latch is what survives that gap. The cached bearer is deliberately
    /// left alone: it still works, and dropping it would break an operation
    /// that would otherwise have succeeded for the rest of the token's life.
    /// Withholding the promotion is the whole of the response.
    ///
    /// It is a statement about the IdP's current verdict, not a headstone: a
    /// later grant releases it. Releasing it promotes nothing by itself — the
    /// connection stays parked until its next accepted operation, which is the
    /// same evidence every other promotion needs. Only a REFUSED GRANT latches
    /// — a throttle, an outage or a transport failure is nobody answering about
    /// the credential and leaves it alone. A refusal that carried a retryable
    /// OAuth code still latches, because no code discriminates permanence; the
    /// cost is that a connection stays parked for the length of a brown-out,
    /// and it un-parks on its next accepted operation.
    ///
    /// This narrows the window rather than closing it, and cannot do better:
    /// the latch records refusals that were *observed*, and a grant is only
    /// attempted on a cache miss or a proactive refresh. A secret rotated a
    /// minute into a one-hour token is refused by nobody for the next ~53
    /// minutes, during which operations are accepted and promotion is
    /// warranted on every piece of evidence that exists. Closing that needs
...
/// Describe an Entra token failure without quoting the body.
///
/// The body is `{"error":"invalid_client","error_description":"AADSTS7000215…",
/// "correlation_id":"<guid>"}`. `error_description` echoes the rejected client
/// assertion, so only two fields escape.
///
/// The two need DIFFERENT validators, which is why this does not run both
/// through `json_code`: `error` is a code token, while `correlation_id` is a
/// GUID and legitimately begins with a digit — the code grammar would reject
/// every real one. The GUID goes through the canonical-shape validator the
/// storage paths already use, so an intermediary cannot widen it into a carrier
/// for credential material.
fn entra_failure_detail(body: &[u8]) -> String {
    ...
    let window = &body[..body.len().min(ovstorage_plugin::provider_error::SCAN_LIMIT)];
    let correlation = serde_json::from_slice::<serde_json::Value>(window)
        .ok()
        .and_then(|value| {
            value
        ...
        .and_then(|raw| crate::error_body::correlation_token(&raw));
    match correlation {
        Some(id) => format!("{detail}; correlation_id={id}"),
        None => detail,
    }
}
...
impl AzureAuth {
    ...
    ///
    /// **Data-path callers want `AzureClient::bearer_token` instead.** A
    /// refused grant here is the only trace the IdP leaves anywhere, and
    /// `AzureClient::bearer_token` is what advances the connection's refusal
    /// epoch on it. Calling this directly from an operation lets an acceptance
    /// earlier in that operation promote a connection whose credential has just
    /// been refused.
    ///
    /// The background refresh loop is the one legitimate direct caller, and it
    /// is covered differently: it belongs to no operation, so no witness's
    /// window contains it, and it keeps the cached bearer on failure — which
    /// the service goes on accepting until that bearer expires.
    /// `credential_refused` is what records a refusal it meets.
    ...
    /// Whether the IdP has refused a grant for this credential and has not
    /// since issued one. Consulted by the connection-promotion veto.
    pub(crate) fn credential_refused(&self) -> bool {
        self.credential_refused.load(Ordering::Relaxed)
    }

    /// Attempt a grant and latch what the IdP said about the credential.
    ///
    /// Every grant attempt in this plugin lands here — the data path through
    /// `bearer_token`, the proactive loop through `refresh_now` — so the latch
    /// cannot miss a refusal, and a later success clears it wherever it comes
    /// from. A transient condition (throttling, an outage) is not a verdict on
    /// the credential and leaves the latch alone.
    async fn fetch_token(&self, client: &Client) -> Result<CachedToken> {
        let outcome = self.request_token(client).await;
        match &outcome {
            Ok(_) => self.credential_refused.store(false, Ordering::Relaxed),
            Err(error) => {
                let reason = match error.context() {
                    Some(ErrorContext::Auth { reason, .. }) => reason.as_deref().unwrap_or(""),
                    _ => "",
                };
                if crate::client::entra_reason_is_grant_refusal(reason) {
    ...
    async fn request_token(&self, client: &Client) -> Result<CachedToken> {
        ...
        if !response.status().is_success() {
            ...
            // The status alone does not say whether the CREDENTIAL was refused:
            // Entra answers a rotated secret and a shed load with the same 400.
            // RFC 6749 s5.2 puts `server_error` and `temporarily_unavailable`
            // in this response shape, and Microsoft directs clients to retry
            // the latter. So carry the OAuth error token — sanitized by the
            // same validator the storage paths use, so an intermediary cannot
            // widen it — and let the classifier read it.
            let reason = match ovstorage_plugin::provider_error::json_code(&body, "/error") {
                Some(code) => format!("entra_status_{}/{code}", status.as_u16()),
                None => format!("entra_status_{}", status.as_u16()),
            };
            return Err(Error::new(
                ErrorCode::AuthRequired,
                format!(
                    "Entra token request returned {status}: {}",
                    entra_failure_detail(&body)
                ),
            )
            .with_context(ErrorContext::Auth {
                connection_id: ConnectionId(String::new()),
...
mod tests {
    ...
    /// Entra's `error_description` echoes the rejected client assertion. Driven
    /// end to end through the real token request against the mock endpoint, so
    /// it pins the CALL SITE and not just the helper.
    #[tokio::test]
    async fn an_entra_failure_does_not_echo_the_provider_body() {
        let body = r#"{"error":"invalid_client","error_description":"AADSTS7000215: Invalid client secret provided. client_assertion=eyJhbGciOi.SECRET","correlation_id":"1b9d6bcd-bbfd-4b2d-9b5d-ab8dfbbd4bed"}"#;
        let response = format!(
            "HTTP/1.1 401 Unauthorized\r\nContent-Type: application/json\r\nContent-Length: {}\r\n\r\n{}",
            body.len(),
            body,
        );
        let (endpoint, _hits) = spawn_mock_entra_endpoint(vec![response]).await;
        let auth = oauth_auth_pointed_at(&endpoint);
        let err = auth
            .bearer_token(&Client::new())
            .await
            .expect_err("a 401 from the IDP is an error");
        ...
        assert!(
            err.message()
                .contains("correlation_id=1b9d6bcd-bbfd-4b2d-9b5d-ab8dfbbd4bed"),
            "the GUID is the handle support asks for: {}",
            err.message()
        );
        for leaked in [
            "AADSTS7000215",
    ...
    /// A body with no recoverable code is reported by length, and a
    /// `correlation_id` that is not a canonical GUID is dropped rather than
    /// reported — an intermediary must not be able to use it as a carrier.
    #[test]
    fn an_entra_body_with_no_code_is_reported_by_length() {
        let detail = entra_failure_detail(b"<html>upstream connect error sig=7hK4wQ2m</html>");
        assert!(detail.contains("byte body suppressed"), "{detail}");
        assert!(!detail.contains("7hK4wQ2m"), "{detail}");

        let hostile = br#"{"error":"invalid_client","correlation_id":"not-a-guid sig=7hK4wQ2m"}"#;
        let detail = entra_failure_detail(hostile);
        assert!(detail.contains("invalid_client"), "{detail}");
        assert!(!detail.contains("correlation_id="), "{detail}");
        assert!(!detail.contains("7hK4wQ2m"), "{detail}");
    }
    ...
    async fn background_refresh_aborts_on_drop() {
        ...
        // cannot upgrade after this point. The abort below
