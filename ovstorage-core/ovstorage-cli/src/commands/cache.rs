...
use crate::commands::util::{byte_cache_state_config, cache_from_loaded_or_env};
use crate::session::SessionState;

pub(crate) fn cache_status(state: &SessionState) -> ovstorage::Result<()> {
    let Some(cache) = cache_from_loaded_or_env(byte_cache_state_config(&state.stack).as_ref())?
    else {
