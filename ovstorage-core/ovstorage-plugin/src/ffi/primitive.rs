...
use super::*;
use crate::ffi::abi_alloc::abi_buffer_free;
...
//
// Buffers come from the ABI heap (see `abi_alloc`), not the Rust global
// allocator, so the side that receives a container can release it
// regardless of which allocator either binary installs.
// ---------------------------------------------------------------------
...
impl Drop for Str {
    fn drop(&mut self) {
        if !self.ptr.is_null() {
            // SAFETY: constructors allocate on the ABI heap with
            // `cap == abi_capacity(len)`.
            unsafe {
                abi_buffer_free(self.ptr as *mut u8, self.len);
            }
...
impl Drop for Bytes {
    fn drop(&mut self) {
        if !self.ptr.is_null() {
            ...
            // `cap == abi_capacity(len)`.
            unsafe {
                abi_buffer_free(self.ptr, self.len);
            }
            self.ptr = std::ptr::null_mut();
            self.len = 0;
        }
