...
pub async fn watch_directory(
    ...
    let prefix_key = address::directory_key(&parsed.key);
...
impl ChangeFeedClient {
    ...
    async fn get_blob(&self, path: &str, cancel: &CancellationToken) -> Result<Vec<u8>> {
        ...
        // Encoded to match the URL above, per Azure's "encoded exactly as it
        // is in the URI" rule for the canonicalized resource — the same
        // reason `canonical_path_for_blob` encodes its key.
        let canonical_path = format!(
            "{}/{CHANGE_FEED_CONTAINER}/{}",
            self.config.change_feed_canonical_prefix(),
            url_encode_path(path)
        );
...
fn map_record(
    ...
    // `key` is decoded, which is what `join_relative` wants: it escapes on the
    // way in, using the same set `canonicalize` uses on the way out. Encoding
    // here as well produced `a%2520b`, an address whose derived key carried the
    // escapes literally.
    let Ok(mut event_address) = address::join_relative(&filter.address_root, &key) else {
        // Skip, never propagate. `Ok(None)` is the channel this function
        // already has, and it lets the chunk offset advance: a propagated
        // error unwinds before `mark_decoded`, so the same non-retryable
        // failure is raised on every subsequent poll and one unaddressable
        // blob name kills the change feed for the whole container.
        tracing::warn!(
            target: "ovstorage.azure.subscription",
            plugin = "azure",
            key = %key,
            "azure: blob name is not addressable as a URI path; change event omitted",
        );
        return Ok(None);
    };
