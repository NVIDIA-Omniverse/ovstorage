...
pub fn capabilities_to_proto(caps: &ovstorage_plugin::Capabilities) -> pb::Capabilities {
    pb::Capabilities {
        ...
        supports_copy: Some(caps.supports_copy),
        supports_rename: Some(caps.supports_rename),
...
pub fn capabilities_from_proto(
    ...
    Ok(ovstorage_plugin::Capabilities {
        ...
        supports_metadata_rewrite_emulation: caps.supports_metadata_rewrite_emulation,
        writes_are_atomic: caps.writes_are_atomic,
        // Absent means an older v2 peer that predates these fields, not a peer
        // asserting `false`. `false` is actionable — a UI greys the operation
        // out — so absence decodes as attemptable and the call decides.
        supports_copy: caps.supports_copy.unwrap_or(true),
        supports_rename: caps.supports_rename.unwrap_or(true),
        supports_server_side_copy: caps.supports_server_side_copy,
        supports_server_side_rename: caps.supports_server_side_rename,
        supports_atomic_rename: caps.supports_atomic_rename,
        has_real_directories: caps.has_real_directories,
...
///
/// `Succeeded` is intentionally lossy on the wire: only `connection.id`
/// propagates. The daemon persists credentials before success, so resolved
/// credential bytes are never embedded in the stream. `RegisterCredential` is
/// reserved for credentials produced by an explicitly external flow.
        ...
        Ev::Failed(failed) => AuthEventPartial::Failed {
            error: failed
                ...
                .map(|detail| {
                    ...
                    if let Some(next_action) = detail.next_action.filter(|s| !s.is_empty()) {
                        error = error.with_next_action(next_action);
                    }
                    error
                })
...
/// Wire shape of `AuthEvent` with `Succeeded` reduced to just `connection_id`.
/// The daemon has already persisted the resolved credential; the SDK pairs the
/// id with the in-flight `Connection` to rebuild a full
/// `AuthEvent::Succeeded`.
#[derive(Clone, Debug)]
...
fn error_code_to_status_code(code: ErrorCode) -> tonic::Code {
    match code {
        ...
        // Deliberately not `Aborted`, which the gRPC spec defines as "the
        // client should retry at a higher level" — for a write whose metadata
        // patch failed, retrying the whole operation re-uploads the object.
        // Not `DataLoss` either: nothing is lost, part of the work landed.
        // `Internal` is the least retry-inviting of the plausible codes — a
        // configured gRPC retry policy can name any status, so no tonic code
        // guarantees a client will not retry. The binding statement is
        // ovstorage's own: `ErrorCode::PartialCompletion` is not retryable.
        // The exact code name rides in the error detail, so a client recovers
        // `PartialCompletion` losslessly and can switch on it.
        ErrorCode::Internal | ErrorCode::PartialCompletion => tonic::Code::Internal,
...
fn error_context_to_proto(context: &ovstorage_plugin::ErrorContext) -> pb::error_detail::Context {
    match context {
        ...
        ovstorage_plugin::ErrorContext::Partial {
            completed,
            failed,
            failed_outcome,
            rollback,
        } => pb::error_detail::Context::Partial(pb::ErrorPartialContext {
            completed: partial_stage_to_proto(*completed) as i32,
            failed: partial_stage_to_proto(*failed) as i32,
            failed_outcome: stage_outcome_to_proto(*failed_outcome) as i32,
            rollback: rollback_effect_to_proto(*rollback) as i32,
        }),
    }
}

fn partial_stage_to_proto(stage: ovstorage_plugin::PartialStage) -> pb::PartialStage {
    match stage {
        ovstorage_plugin::PartialStage::ObjectData => pb::PartialStage::ObjectData,
        ovstorage_plugin::PartialStage::UserMetadata => pb::PartialStage::UserMetadata,
        ovstorage_plugin::PartialStage::SourceRemoval => pb::PartialStage::SourceRemoval,
    }
}

/// `None` on `Unspecified` — see the proto: zero is the value an omitted field
/// decodes to, so it must never map onto a real stage.
fn partial_stage_from_proto(stage: pb::PartialStage) -> Option<ovstorage_plugin::PartialStage> {
    match stage {
        pb::PartialStage::Unspecified => None,
        pb::PartialStage::ObjectData => Some(ovstorage_plugin::PartialStage::ObjectData),
        pb::PartialStage::UserMetadata => Some(ovstorage_plugin::PartialStage::UserMetadata),
        pb::PartialStage::SourceRemoval => Some(ovstorage_plugin::PartialStage::SourceRemoval),
    }
}

fn stage_outcome_to_proto(outcome: ovstorage_plugin::StageOutcome) -> pb::StageOutcome {
    match outcome {
        ovstorage_plugin::StageOutcome::NotApplied => pb::StageOutcome::NotApplied,
        ovstorage_plugin::StageOutcome::Unknown => pb::StageOutcome::Unknown,
    }
}

/// `None` on `Unspecified`; see [`partial_stage_from_proto`].
fn stage_outcome_from_proto(outcome: pb::StageOutcome) -> Option<ovstorage_plugin::StageOutcome> {
    match outcome {
        pb::StageOutcome::Unspecified => None,
        pb::StageOutcome::NotApplied => Some(ovstorage_plugin::StageOutcome::NotApplied),
        pb::StageOutcome::Unknown => Some(ovstorage_plugin::StageOutcome::Unknown),
    }
}

fn rollback_effect_to_proto(effect: ovstorage_plugin::RollbackEffect) -> pb::RollbackEffect {
    match effect {
        ovstorage_plugin::RollbackEffect::RestoresPriorState => {
            pb::RollbackEffect::RestoresPriorState
        }
        ovstorage_plugin::RollbackEffect::DestroysRequestedWork => {
            pb::RollbackEffect::DestroysRequestedWork
        }
    }
}

/// `None` on `Unspecified`; see [`partial_stage_from_proto`]. This is the field
/// the rejection exists for: zero must not read as "rollback is safe".
fn rollback_effect_from_proto(
    effect: pb::RollbackEffect,
) -> Option<ovstorage_plugin::RollbackEffect> {
    match effect {
        pb::RollbackEffect::Unspecified => None,
        pb::RollbackEffect::RestoresPriorState => {
            Some(ovstorage_plugin::RollbackEffect::RestoresPriorState)
        }
        pb::RollbackEffect::DestroysRequestedWork => {
            Some(ovstorage_plugin::RollbackEffect::DestroysRequestedWork)
        }
    }
}
...
fn error_context_from_proto(
    ...
    match context {
        ...
        // Every field must decode to a real value. Two ways it can fail, and
        // both drop the whole context rather than guess:
        //
        // - `try_from` rejects a number this build does not know (a newer
        //   peer);
        // - `Unspecified` is rejected explicitly, because proto3 has no field
        //   presence for scalars, so an omitted or truncated field arrives as
        //   zero. Reserving zero and refusing it is what stops a malformed
        //   message from asserting `RestoresPriorState` — that is, telling the
        //   caller it is safe to delete data that is already durable.
        //
        // Dropping the context loses a hint; guessing it wrong loses data.
        pb::error_detail::Context::Partial(partial) => {
            let completed = pb::PartialStage::try_from(partial.completed).ok()?;
            let failed = pb::PartialStage::try_from(partial.failed).ok()?;
            let failed_outcome = pb::StageOutcome::try_from(partial.failed_outcome).ok()?;
            let rollback = pb::RollbackEffect::try_from(partial.rollback).ok()?;
            Some(ovstorage_plugin::ErrorContext::Partial {
                completed: partial_stage_from_proto(completed)?,
                failed: partial_stage_from_proto(failed)?,
                failed_outcome: stage_outcome_from_proto(failed_outcome)?,
                rollback: rollback_effect_from_proto(rollback)?,
            })
        }
    }
}
...
fn redirect_credential_to_proto(
    credential: ovstorage_plugin::RedirectCredential,
) -> pb::RedirectCredential {
    match credential {
        ovstorage_plugin::RedirectCredential::Unspecified => pb::RedirectCredential::Unspecified,
        ovstorage_plugin::RedirectCredential::None => pb::RedirectCredential::None,
        ovstorage_plugin::RedirectCredential::Request => pb::RedirectCredential::Request,
        ovstorage_plugin::RedirectCredential::Connection => pb::RedirectCredential::Connection,
    }
}

/// A value this build does not know — including the zero an older peer sends
/// by omitting the field — decodes to
/// [`ovstorage_plugin::RedirectCredential::Unspecified`], which a host treats
/// as broadly as `Connection`. Refusing the message instead would break
/// replicas over a field whose fail-safe reading is well defined.
fn redirect_credential_from_proto(value: i32) -> ovstorage_plugin::RedirectCredential {
    match pb::RedirectCredential::try_from(value) {
        Ok(pb::RedirectCredential::None) => ovstorage_plugin::RedirectCredential::None,
        Ok(pb::RedirectCredential::Request) => ovstorage_plugin::RedirectCredential::Request,
        Ok(pb::RedirectCredential::Connection) => ovstorage_plugin::RedirectCredential::Connection,
        Ok(pb::RedirectCredential::Unspecified) | Err(_) => {
            ovstorage_plugin::RedirectCredential::Unspecified
        }
    }
}
...
fn redirect_scope_to_proto(scope: &RedirectScope) -> pb::RedirectScope {
    pb::RedirectScope {
        ...
        credential: redirect_credential_to_proto(scope.credential) as i32,
    }
}
...
fn redirect_scope_from_proto(
    ...
    Ok(RedirectScope {
        ...
        credential: redirect_credential_from_proto(scope.credential),
    })
}
...
fn error_code_from_name(name: &str) -> Option<ErrorCode> {
    match name {
        ...
        "PartialCompletion" => Some(ErrorCode::PartialCompletion),
        _ => None,
    }
}
...
mod tests {
    ...
    fn capability_metadata_header_name_is_x_ov_iauth() {
        // Lowercase ASCII per HTTP/2 wire encoding; pin the exact header name.
    ...
    fn read_redirect_round_trips_non_default_response_parsing() {
        ...
        let redirect = ovstorage_plugin::ReadRedirect {
            ...
            scope: RedirectScope {
                ...
                expires_at: SystemTime::UNIX_EPOCH + Duration::from_secs(1_000),
                credential: ovstorage_plugin::RedirectCredential::None,
            },
    ...
    /// A peer built before `RedirectScope.credential` existed omits the field,
    /// which decodes as zero. That must land on the fail-safe arm — a host
    /// treats `Unspecified` as broadly as `Connection` — rather than on the
    /// delegable `None`.
    #[test]
    fn omitted_credential_decodes_as_unspecified() {
        let proto = pb::RedirectScope {
            physical_url_prefix: "https://example.com".into(),
            operations: Some(access_ops_to_proto(&AccessOps::default())),
            expires_at_unix_millis: 1_000_000,
            // Left at the wire default an older peer produces.
            ..Default::default()
        };
        assert_eq!(proto.credential, 0);
        let scope = redirect_scope_from_proto(Some(proto)).unwrap();
        assert_eq!(
            scope.credential,
            ovstorage_plugin::RedirectCredential::Unspecified
        );
        assert!(!scope.credential.is_request_scoped());
    }

    #[test]
    fn credential_round_trips_through_proto() {
        for credential in [
            ovstorage_plugin::RedirectCredential::Unspecified,
            ovstorage_plugin::RedirectCredential::None,
            ovstorage_plugin::RedirectCredential::Request,
            ovstorage_plugin::RedirectCredential::Connection,
        ] {
            ...
            let back = redirect_scope_from_proto(Some(redirect_scope_to_proto(&scope))).unwrap();
            assert_eq!(back.credential, credential);
        }
    }
    ...
    /// Both motivating shapes survive the broker hop end to end, and stay
    /// distinguishable on the far side — they want opposite remedies, so a
    /// projection that collapsed them would be worse than no context.
    #[test]
    fn error_context_partial_round_trips_both_shapes() {
        use ovstorage_plugin::{ErrorContext, PartialStage, RollbackEffect, StageOutcome};

        let shapes = [
            // Metadata after a committed write: re-issue the patch, never roll back.
            ...
            // A rename emulated as copy-then-delete whose delete failed. No
            // in-tree producer emits this yet; it is the case the general code
            // exists for, so the wire must carry it.
            ErrorContext::Partial {
                completed: PartialStage::ObjectData,
        ...
        // The two shapes have opposite `next_action` strings; both must survive
        // the hop so a brokered caller can distinguish them without reading the
        // partial payload.
        let next_actions = [
            "Re-apply the user metadata with update_metadata.",
            "The source object no longer exists; the destination holds the bytes.",
        ];

        let mut decoded_shapes = Vec::new();
        for (shape, na) in shapes.iter().zip(next_actions.iter()) {
            let error = Error::new(ErrorCode::PartialCompletion, "object committed")
                .with_context(shape.clone())
                .with_next_action(*na);
            let decoded = status_to_error(error_to_status(error));
            assert_eq!(decoded.code(), ErrorCode::PartialCompletion);
            assert_eq!(
                decoded.context(),
                Some(shape),
                "partial context did not survive the broker hop intact",
            );
            assert_eq!(
                decoded.next_action(),
                Some(*na),
                "next_action did not survive the broker hop",
            );
            decoded_shapes.push(decoded.context().cloned());
        }
        assert_ne!(
            decoded_shapes[0], decoded_shapes[1],
            "the two shapes must stay distinguishable across the wire",
        );
    }

    /// proto3 has no field presence for scalars, so an OMITTED field arrives
    /// as zero. Zero is reserved `UNSPECIFIED` on all three enums and rejected,
    /// which is what stops a truncated or malformed message from asserting
    /// `RestoresPriorState` — telling a caller it is safe to delete data that
    /// is already durable.
    #[test]
    fn an_omitted_rollback_field_drops_the_context_rather_than_reading_as_safe() {
        // Default-constructed: every field is zero, exactly what a message
        // with those fields omitted decodes to.
        let omitted = pb::error_detail::Context::Partial(pb::ErrorPartialContext::default());
        assert_eq!(
            error_context_from_proto(omitted),
            None,
            "an omitted rollback must not decode as RestoresPriorState",
        );

        // Only `rollback` omitted, the rest valid — the narrow case.
        let partial_omission = pb::error_detail::Context::Partial(pb::ErrorPartialContext {
            completed: pb::PartialStage::ObjectData as i32,
            failed: pb::PartialStage::UserMetadata as i32,
            failed_outcome: pb::StageOutcome::NotApplied as i32,
            rollback: 0,
        });
        assert_eq!(error_context_from_proto(partial_omission), None);

        // Control: the same message with rollback set does decode, so the
        // assertions above are about the omission and not about the shape.
        let complete = pb::error_detail::Context::Partial(pb::ErrorPartialContext {
            completed: pb::PartialStage::ObjectData as i32,
            failed: pb::PartialStage::UserMetadata as i32,
            failed_outcome: pb::StageOutcome::NotApplied as i32,
            rollback: pb::RollbackEffect::DestroysRequestedWork as i32,
        });
        assert!(error_context_from_proto(complete).is_some());
    }

    /// Zero must not be a real value on any of the three enums, or the
    /// omission above becomes an assertion again for that field.
    #[test]
    fn zero_is_unspecified_on_every_partial_enum() {
        assert_eq!(pb::PartialStage::Unspecified as i32, 0);
        assert_eq!(pb::StageOutcome::Unspecified as i32, 0);
        assert_eq!(pb::RollbackEffect::Unspecified as i32, 0);
        assert!(partial_stage_from_proto(pb::PartialStage::Unspecified).is_none());
        assert!(stage_outcome_from_proto(pb::StageOutcome::Unspecified).is_none());
        assert!(rollback_effect_from_proto(pb::RollbackEffect::Unspecified).is_none());
    }

    /// A proto3 enum is an `i32` on the wire. A value from a newer peer must
    /// NOT decode as the zero variant either.
    #[test]
    fn an_unknown_partial_enum_value_drops_the_context_rather_than_guessing() {
        let unknown = pb::error_detail::Context::Partial(pb::ErrorPartialContext {
            completed: pb::PartialStage::ObjectData as i32,
            failed: pb::PartialStage::SourceRemoval as i32,
            failed_outcome: pb::StageOutcome::Unknown as i32,
            // Not a value this build knows.
            rollback: 9_999,
        });
        assert_eq!(
            error_context_from_proto(unknown),
            None,
            "an unrecognised rollback value must drop the context, not fall \
             back to the zero variant",
        );

        // Control: the same message with a known value does decode, so the
        // assertion above is about the unknown value and not about the shape.
        let known = pb::error_detail::Context::Partial(pb::ErrorPartialContext {
            completed: pb::PartialStage::ObjectData as i32,
            failed: pb::PartialStage::SourceRemoval as i32,
            failed_outcome: pb::StageOutcome::Unknown as i32,
            rollback: pb::RollbackEffect::RestoresPriorState as i32,
        });
        assert!(error_context_from_proto(known).is_some());
    }

    /// `Aborted` invites a higher-level retry, which for a committed write
    /// means re-uploading the object. Pin the choice so it cannot drift.
    #[test]
    fn partial_completion_is_not_a_retry_inviting_grpc_code() {
        let code = error_code_to_status_code(ErrorCode::PartialCompletion);
        assert_eq!(code, tonic::Code::Internal);
        assert_ne!(code, tonic::Code::Aborted);
        assert_ne!(code, tonic::Code::Unavailable);
        assert_ne!(code, tonic::Code::DataLoss);
    }

    /// The tonic code is coarse, so the exact name is what a client switches
    /// on. Assert the string survives in both directions.
    #[test]
    fn partial_completion_name_round_trips() {
        let error = Error::new(ErrorCode::PartialCompletion, "object committed");
        let decoded = status_to_error(error_to_status(error));
        assert_eq!(decoded.code(), ErrorCode::PartialCompletion);
    }
