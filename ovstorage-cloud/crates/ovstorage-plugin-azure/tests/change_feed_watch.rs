...
use ovstorage_plugin_azure::AzureBackendFactory;
...
async fn open_watch(endpoint: &str, opts: WatchDirectoryOptions) -> BackendChangeStream {
    let instance = AzureBackendFactory::new()
        .instantiate(&connection_request(endpoint), None)
        .await
        .expect("instantiate Azure backend");
    let target = ResolvedTarget {
        backend_id: BackendId(instance.backend_id.0.clone()),
