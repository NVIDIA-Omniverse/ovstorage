...
//!
//! The tests drive the public `NucleusBackendFactory::instantiate`
//! path (the SPI factory). `instantiate` itself does not authenticate
//! — every refusal-on-precondition check fires SYNCHRONOUSLY at the
//! SPI entry point, BEFORE any wire or auth interaction; that is the
//! contract under test. No real WebSocket / HTTP server is required.
...
use ovstorage_plugin::{
    BackendId, ByteRange, ConfigValue, ConnectionRequest, CopyOptions, DeleteOptions, ErrorCode,
    ListOptions, ListVersionsOptions, ReadOptions, RenameOptions, ResolvedTarget, SecretBundle,
    Url, WriteOptions, address,
};
use ovstorage_plugin_nucleus::NucleusBackendFactory;
...
async fn nucleus_backend() -> Arc<dyn Backend> {
    ...
    NucleusBackendFactory::new()
        .instantiate(&request, None)
        .await
        .expect("factory instantiate succeeds without auth")
        .backend
}
...
async fn read_range_inverted_returns_invalid_argument() {
    ...
    // refusal. The workspace builds with `panic = "abort"`, so an
    // inverted range from a buggy caller is best surfaced as a typed
    // error rather than lumped into a less-specific Unsupported case.
    let backend = nucleus_backend().await;
    let err = backend
        .read(
            obj("/Users/alice/x"),
