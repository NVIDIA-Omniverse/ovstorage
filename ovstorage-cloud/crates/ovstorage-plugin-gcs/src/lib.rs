...
pub struct GcsBackendFactory;

impl GcsBackendFactory {
    pub fn new() -> Self {
        Self
    }
}

impl Default for GcsBackendFactory {
    fn default() -> Self {
        Self::new()
    }
}
...
impl shim::Factory for GcsBackendFactory {
    ...
    async fn instantiate(
        ...
        let authenticator = Authenticator::new(&request.credentials, http.clone())?;
        let auth_state = if authenticator.has_eager_credentials() {
        ...
        // Proactively refresh the OAuth bearer at ~90% of TTL so
        // long-lived processes don't hit `Unauthenticated` mid-RPC.
        // No-op for anonymous credentials. The task holds only a
        // `Weak<Authenticator>` and aborts on Drop.
        ...
        Ok(shim::BackendInstance {
            backend_id: BackendId(format!("gcs:{}", config.address_root)),
            backend,
            address_roots: vec![AddressRoot {
                address: config.address_root.clone(),
                display_name: None,
                backend_kind: "gcs".into(),
                connection_id: None,
                capabilities: gcs_capabilities(&config),
            ...
            display_name: request
                .display_name
                .clone()
                .or_else(|| Some(format!("Google Cloud Storage: {}", config.bucket))),
            auth_state,
        })
    }
...
#[async_trait::async_trait]
impl shim::Backend for GcsBackend {
    async fn stat(
        &self,
        ...
        race_cancel(
            ...
            async move {
                ...
                if self.auth.is_anonymous() {
                    ...
                    return Ok(ReadResult::Redirect(read_redirect(url, host, extras)));
                }
    ...
    async fn continue_write(
        ...
        // `.../sessionXXX` match a sibling `.../sessionXXXextra`.
        ...
        let object: GcsObject = serde_json::from_slice(&result.captured_body).map_err(|err| {
            Error::new(
                ErrorCode::Internal,
                format!("GCS resumable response was not Object JSON: {err}"),
            )
        })?;
            ...
            async move {
                ...
                loop {
                    ...
                    if !target.object.is_empty() {
                        query.push(("prefix".to_string(), target.object.clone()));
                    }
                    if !opts.recursive {
                    ...
                    for object in &listing.items {
                        ...
                        if is_marker {
                            let address = address::join_relative(&self.config.address_root, name)?;
    ...
    async fn rename(
        ...
        race_cancel(
            ...
            async move {
                ...
                let response = send(delete_request).await?;
                if response.status() == reqwest::StatusCode::NOT_FOUND {
                    return Ok(());
                }
                if let Err(delete_error) = ensure_success(response).await {
                    ...
                    .await?;
                    if let Err(rollback) = ensure_success(rollback_response).await {
...
impl GcsBackend {
    ...
    async fn put_object_inline(
        ...
        let request = self
            ...
            .body(body);
        let response = ensure_success(send(request).await?).await?;
        let object_resource: GcsObject = decode_json(response).await?;
    ...
    async fn stream_resumable_upload(
        ...
        let initiate = self
            ...
            .json(&serde_json::Value::Object(metadata_payload));
        let response = ensure_success(send(initiate).await?).await?;
        let session_url = response
            .headers()
            .get(reqwest::header::LOCATION)
            .and_then(|v| v.to_str().ok())
    ...
    async fn rewrite_object(
        ...
        loop {
            ...
            let response = send(request).await?;
            let body: RewriteResponse = decode_json(response).await?;
            if body.done {
...
/// with `InvalidArgument` — the workspace uses `panic = "abort"`, so
/// an inverted slice on the host-side follower would terminate the
/// worker if we let one through.
...
fn read_redirect(url: String, host: String, extra_headers: Vec<(String, String)>) -> ReadRedirect {
...
async fn decode_json<T: for<'de> serde::Deserialize<'de>>(
    ...
    serde_json::from_str(&body).map_err(|err| {
        Error::new(
            ErrorCode::Internal,
            format!("GCS response was not JSON: {err} (body: {body})"),
        )
    })
}
...
fn map_status_to_error(status: u16, body: &str) -> Error {
    ...
    if status == 401 {
        return Error::new(
            ...
            format!("GCS request requires authentication (HTTP 401): {body}"),
        )
    ...
    Error::new(code, format!("GCS returned HTTP {status}: {body}"))
}
...
fn parse_gcs_address(addr: &Url) -> Result<GcsObjectRef> {
    let raw = addr.as_str();
    let Some(rest) = raw.strip_prefix("gs://") else {
        return Err(Error::new(
            ErrorCode::InvalidArgument,
    ...
    let (bucket, suffix) = rest.split_at(authority_end);
    if bucket.is_empty() {
        return Err(Error::new(
            ErrorCode::InvalidArgument,
            "GCS bucket is empty",
    ...
    let (path, selector) = split_selector(suffix);
    let object = path.strip_prefix('/').unwrap_or(path);
...
ovstorage_plugin::ovstorage_plugin!(GcsBackendFactory::default);
...
mod tests {
    ...
    fn descriptor_reports_native_gcs_schema() {
        let descriptor = GcsBackendFactory::new().descriptor();
    ...
    #[tokio::test]
    async fn instantiate_reports_native_capabilities() {
        let instance = GcsBackendFactory::new()
            .instantiate(&request("asset-bucket"), None)
            .await
            .unwrap();
        let root = &instance.address_roots[0];
        assert_eq!(root.address.as_str(), "gs://asset-bucket/");
        assert!(root.capabilities.supports_native_metadata_patch);
        assert!(!root.capabilities.supports_watch_directory);
    }

    #[tokio::test]
    async fn instantiate_reports_watch_capabilities_when_pubsub_is_configured() {
        ...
        let instance = GcsBackendFactory::new()
            .instantiate(&req, None)
            .await
            .unwrap();
        let caps = &instance.address_roots[0].capabilities;
    ...
    fn parser_extracts_object_path_and_selector() {
        // url::Url normalizes RFC 3986 dot-segments before this parser
        // sees the input; percent-encoded literals stay encoded.
        ...
        assert_eq!(
            ...
            GcsObjectRef {
                ...
                object: "a/b.%2F".into(),
                selector: Some("?generation=7&x=y#frag".into()),
            }
        );
    ...
    fn read_range_header_rejects_inverted_range() {
        // panic = "abort" would terminate the worker if an inverted
        // range made it to a downstream slice. Reject at construction.
    ...
    #[tokio::test]
    async fn continue_write_rejects_target_address_mismatch() {
