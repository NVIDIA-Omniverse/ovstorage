    ...
    loop {
        ...
        match Cli::try_parse_from(&argv) {
            Ok(cli) => match cli.command {
                Some(command) => {
                    // The REPL owns its own double-tap window per command: each prompt is a
                    // fresh gesture, unlike the startup -> single-command path.
                    let mut interrupt_at: Option<std::time::Instant> = None;
                    if let Err(err) = dispatch_with_cancel(command, state, &mut interrupt_at).await
                    {
