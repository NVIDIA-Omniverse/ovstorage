...
use ovstorage::{
    AccessOps, BodyStream, ByteRange, CancellationToken, ChangeEvent, ChangeKind, Error, ErrorCode,
    IfDestExists, Library, ObjectInfo, ReadOptions, StateConfig, Storage, Url, address,
};
...
pub(crate) async fn stream_read_to_writer<W: Write>(
    ...
    let (mut stream, _) = lib.read_stream(addr, opts, Some(cancel.clone())).await?;
...
pub(crate) async fn stream_read_to_path(
    ...
    stream_read_to_writer(lib, addr, opts, &mut writer, cancel).await
}
...
///   etag matches `<etag>` (`ObjectModified` otherwise).
///
