...
use ovstorage::{CancellationToken, Error, ErrorCode};

use crate::commands::util::OutputFormat;
...
pub(crate) enum Command {
    ...
    StateStatus,
    /// Aggregate Stack diagnostic state into a single report.
    Doctor {
    ...
    /// Set the current directory for resolving relative paths in subsequent
    ...
    Reauth {
        /// Connection display name (matched against `Stack::list_connections`).
        name: String,
    },
    ...
    WriteConfig {
        ...
        /// otherwise. `${ENV_VAR}` refs you chose at connect and refs from the
    ...
    WatchDirectory {
        ...
        /// Resume from a cursor emitted earlier.
...
async fn run() -> ovstorage::Result<()> {
    ...
    // Startup composes the configured connections, and applying one waits for
    // the route-table catch-up that makes it routable. A backend that never
    // answers holds that wait for the library's 30-second bound, which is a very
    // long time to stare at a CLI that has printed nothing. Ctrl+C owns the same
    // exit here as it does mid-command.
    let startup_cancel = CancellationToken::new();
    // One double-tap window spanning startup AND dispatch: a press during the
    // build and a second while the command runs are the same gesture to the
    // user, and the second must force-exit.
    let mut interrupt_at: Option<std::time::Instant> = None;
    let stack = with_interrupt(
        build_stack(&loaded, Some(startup_cancel.clone())),
        &startup_cancel,
        &mut interrupt_at,
    )
    .await?;
    // Honour the press even if the build finished anyway. Nothing on the build
    // path is obliged to turn a fired token into an error -- `build_with_cancel`
    // has no cancellation checkpoint of its own, and the built-in factories
    // ignore the token entirely -- so a build that completes before any layer
    // observes it would otherwise fall through and run the very command the
    // user just tried to abort.
    if startup_cancel.is_cancelled() {
        return Err(Error::new(
            ErrorCode::Cancelled,
            "startup was interrupted before the command ran",
        ));
    }
    let mut state = session::SessionState::build(stack, loaded).await?;

    let Some(command) = cli.command else {
        return commands::repl::run(&mut state).await;
    };

    dispatch_with_cancel(command, &mut state, &mut interrupt_at).await
}
...
) -> ovstorage::Result<ovstorage::StackConfig> {
    if no_config {
        return Ok(ovstorage::StackConfig::default());
    }
    if let Some(path) = explicit {
        return ovstorage::StackConfig::from_toml_path(path);
    }
    if let Some(path) = std::env::var_os("OVSTORAGE_CONFIG") {
        return ovstorage::StackConfig::from_toml_path(Path::new(&path));
    }
    Ok(ovstorage::StackConfig::from_default_path()?.unwrap_or_default())
}
...
pub(crate) async fn dispatch(
    ...
    match command {
        Command::ListRoutes => commands::connect::list_routes(state).await?,
        ...
        Command::Doctor { json } => commands::doctor::run(state.stack.clone(), json).await?,
        Command::Stat {
            address,
            full_metadata,
...
pub(crate) async fn dispatch_with_cancel(
    ...
    with_interrupt(dispatch(command, state, &token), &token, previous).await
}

/// Drive `operation` under the Ctrl+C double-tap state machine: the first press
/// cancels `token` cooperatively and lets the operation report its own outcome;
/// a second press within `interrupt::INTERRUPT_WINDOW` force-exits with code
/// 130. Shared by startup composition and command dispatch so both answer
/// Ctrl+C the same way.
...
async fn build_stack(
...
) -> ovstorage::Result<Arc<ovstorage::Stack>> {
    ...
    ovstorage::host::build_stack_with_cancel(config, factories, cancel).await
}

fn cli_auth_state_root() -> PathBuf {
...
/// this crate's `README.md` (exit-code table). Codes not in the spec
/// table fall through to `1` (generic error).
fn exit_code(code: ErrorCode) -> i32 {
    match code {
        ...
        ErrorCode::Locked => 18,
        // Its own number rather than the generic `1`: an operator scripting a
        // write must be able to tell "nothing happened" from "the object is
        // committed and a later stage is not", because the remedies differ
        // and one of them is destructive.
        ErrorCode::PartialCompletion => 19,
        _ => 1,
    }
}
...
mod tests {
    ...
    fn exit_code_maps_documented_codes() {
        ...
        // Its own number, not the generic 1: an operator scripting a write
        // must be able to tell "nothing happened" from "the object is
        // committed and a later stage is not", because one remedy is
        // destructive. Documented in this crate's README exit-code table.
        assert_eq!(exit_code(ErrorCode::PartialCompletion), 19);
        assert_ne!(exit_code(ErrorCode::PartialCompletion), 1);
    }

    #[test]
    fn partial_completion_has_its_own_error_name() {
        // Falls to "Error" without an explicit arm, which would be
        // indistinguishable from any unclassified code in CLI output.
        assert_eq!(
            error_code_name(ErrorCode::PartialCompletion),
            "PartialCompletion"
        );
        assert_ne!(error_code_name(ErrorCode::PartialCompletion), "Error");
    }
