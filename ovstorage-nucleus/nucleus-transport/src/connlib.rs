...
impl ConnLibTransport {
    ...
    async fn connect_inner(url: &str) -> Result<Self> {
        ...
        // Callers reach this with a URL they built, and a query parameter is
        // the shape a credential arrives in. `connect_async` gets the URL as
        // given; only the field value is redacted.
        let logged_url = redact_url(url);
        info!(conn_id = %conn_id, url = %logged_url, "connecting to nucleus");
        ...
        loop {
            ...
            match next {
                Some(Ok(Message::Binary(data))) => {
                    ...
                    // Response envelopes carry credential fields, so the body
                    // stays out of the event and only its length is reported.
                    trace!(conn_id = %conn_id, id = meta.id, json_len = json.len(), blob_len, fin = meta.fin, stopped = meta.stopped, "received response");
...
impl Transport for ConnLibTransport {
    ...
    async fn send(
        ...
        // `authorize_token` carries the access token in its request body, so
        // the body stays out of the event and only its length is reported.
        trace!(conn_id = %self.conn_id, id, method, json_len, blob_len, "sending request");
