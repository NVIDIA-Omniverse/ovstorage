...
/// The subscriber filter applied when neither `OVSTORAGE_LOG` nor `RUST_LOG` is
/// set: warnings and above from everything, plus `info` and above from
/// `ovstorage`-prefixed targets. `EnvFilter` matches targets by prefix, so a
/// target under that prefix emitting below `info` is dropped by default —
/// operator-facing diagnostics must be `warn` or above to be seen without an
/// explicit override.
pub const DEFAULT_LOG_FILTER: &str = "warn,ovstorage=info";

/// # Errors
///
/// - [`ErrorCode::Transient`] — OTLP trace exporter could not be created
///   (e.g. network or configuration error).
/// - [`ErrorCode::AlreadyExists`] — tracing has already been initialized in
///   this process.
...
/// Used by `load_plugins_from_dir` to skip a cdylib whose manifest/init
/// advertises an ABI this host does not implement (a stale, foreign, or
/// future-version plugin left in the directory). The v2 loader reports these
/// as `IncompatibleType`, but it uses that code for structural corruption too;
/// match the two exact version-check messages so malformed current-ABI plugins
/// still abort discovery.
pub(crate) fn incompatible_abi_plugin(error: &Error) -> bool {
    error.code() == ErrorCode::IncompatibleType
        && matches!(
            error.message(),
            crate::loaded_v2::MANIFEST_ABI_MISMATCH_MESSAGE
                | crate::loaded_v2::INIT_ABI_MISMATCH_MESSAGE
                | crate::loaded_v2::PLUGIN_VTABLE_ABI_MISMATCH_MESSAGE
        )
}

/// Matches `libovstorage_plugin_*.{so,dylib}` on Unix, `ovstorage_plugin_*.dll`
/// on Windows -- per platform, not the union of both.
pub(crate) fn is_plugin_artifact(path: &std::path::Path) -> bool {
    ...
    // Gated per platform, not `unix_ok || windows_ok`. A `.dll` sitting in a
    // Linux plugin directory -- a stale artefact from a cross-build, or a
    // release tree carrying both -- is not loadable here, and accepting it as
    // a candidate means `dlopen`ing a file that only resembles a plugin. That
    // runs its initializers in this process, which is a worse outcome than not
    // finding a plugin. The documented shape and this predicate are the same
    // rule stated twice, so they must not diverge.
    #[cfg(windows)]
    {
        ext == "dll" && stem.starts_with("ovstorage_plugin_")
    }
    #[cfg(not(windows))]
    {
        matches!(ext, "so" | "dylib") && stem.starts_with("libovstorage_plugin_")
    }
}
...
mod tests {
    ...
    /// The three bulk-discovery skip classifiers must partition the error space
    /// correctly: each recognizes only its own case, and a genuine load failure
    /// (e.g. a panicking plugin) is recognized by none — so it still aborts the
    /// scan rather than being silently swallowed.
    #[test]
    fn bulk_discovery_classifiers_partition_load_errors() {
        // Adjacent non-plugin dylib (no manifest symbol).
        let missing = Error::new(
            ErrorCode::InvalidArgument,
            "plugin manifest symbol is missing",
        );
        assert!(missing_plugin_symbol(&missing));
        assert!(!policy_rejected_plugin(&missing));
        assert!(!incompatible_abi_plugin(&missing));

        // Host policy opt-out (`test_only` without `allow_test_plugins`).
        let rejected = Error::new(ErrorCode::PluginRejected, "test_only plugin rejected");
        assert!(policy_rejected_plugin(&rejected));
        assert!(!missing_plugin_symbol(&rejected));
        assert!(!incompatible_abi_plugin(&rejected));

        // Stale/foreign/future-ABI cdylib — what the v2 loader reports
        // (`loaded_v2::read_v2_manifest` / init check). Must be SKIPPED, not
        // abort the scan.
        let foreign_abi = Error::new(
            ErrorCode::IncompatibleType,
            crate::loaded_v2::MANIFEST_ABI_MISMATCH_MESSAGE,
        );
        assert!(incompatible_abi_plugin(&foreign_abi));
        assert!(!missing_plugin_symbol(&foreign_abi));
        assert!(!policy_rejected_plugin(&foreign_abi));

        // A current-ABI plugin with a truncated vtable is broken, not merely
        // incompatible, and must still abort the directory scan.
        let corrupt_current = Error::new(
            ErrorCode::IncompatibleType,
            "v2 plugin PluginVTableV1 struct_size is too small",
        );
        assert!(!incompatible_abi_plugin(&corrupt_current));

        // A genuine load failure is recognized by none → bulk discovery still
        // aborts (a broken plugin the operator should hear about).
        let genuine = Error::new(ErrorCode::Internal, "plugin panicked during init");
        assert!(!missing_plugin_symbol(&genuine));
        assert!(!policy_rejected_plugin(&genuine));
        assert!(!incompatible_abi_plugin(&genuine));
    }
}
