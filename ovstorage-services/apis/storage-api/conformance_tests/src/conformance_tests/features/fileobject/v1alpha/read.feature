Feature: Read() and ReadFromAddress()
  ...
  Background:
    Given a new test namespace called 'read_test_alpha'
  ...
  # In S3 and Azure, a deleted object still retains its identity, so they do not return NotFound.
  # Test marked optional now, depending on whether the test data generator is capable of permanently deleting something.
  @optional
