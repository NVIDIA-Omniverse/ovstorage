...
use ovstorage::{
    DeleteDirectoryOptions, DeleteOptions, Error, ErrorCode, ListOptions, Storage, Url,
};
use rmcp::ErrorData;
...
pub struct DeleteParams {
    ...
    /// `update_metadata.if_match`. Mismatch surfaces `ObjectModified`.
...
impl OvstorageServer {
    ...
    pub async fn ovstorage_delete_directory(
        ...
        let outcome = async {
            let addr = ovstorage::address::parse(&params.address)?;
            if params.dry_run {
                let plan = collect_delete_plan(self.library(), addr, params.recursive).await?;
            ...
            if params.recursive {
                let plan = collect_delete_plan(self.library(), addr.clone(), true).await?;
...
async fn collect_delete_plan(
    ...
    if recursive {
        collect_recursive_delete_plan(library, root).await
    } else {
        let mut objects = Vec::new();
        let mut directories = Vec::new();
        collect_one_directory(library, root, false, &mut objects, &mut directories).await?;
        Ok(DeletePlan {
            objects,
            directories,
        })
...
async fn collect_recursive_delete_plan(
    library: &ovstorage::Library,
    root: Url,
) -> ovstorage::Result<DeletePlan> {
    let mut objects = Vec::new();
    ...
    while let Some(dir) = pending.pop() {
        ...
        collect_one_directory(library, dir, true, &mut objects, &mut directories).await?;
