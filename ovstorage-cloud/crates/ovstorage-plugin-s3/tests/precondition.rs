...
use ovstorage_plugin::{
    ...
    WriteOptions, address, shim::Backend,
};
...
async fn write_redirect_unknown_size_returns_unsupported() {
    ...
    // multipart). Previously fell back to `S3_PUTOBJECT_MAX_BYTES`
    // and produced mismatches at the follower; now refuses so the
    // host routes through write_stream instead.
    let backend = build_backend("http://127.0.0.1:1", "bkt");
    let err = backend
        .write_redirect(
            target("bkt", "x"),
