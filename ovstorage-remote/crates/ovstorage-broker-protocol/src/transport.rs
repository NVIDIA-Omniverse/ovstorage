...
use ovstorage_plugin::{
    ...
    DeleteDirectoryOptions, DeleteOptions, ListOptions, ListPage, ListVersionsOptions, ObjectInfo,
    ReadOptions, ReadResult, RedirectResultBatch, RenameOptions, Result, StatOptions,
    UpdateMetadataOptions, Url, WatchDirectoryOptions, WriteOptions, WriteRedirectBatch,
...
pub trait BrokerClientTransport: Send + Sync {
    ...
    /// Open the streaming `Auth` RPC for `address`. Broker emits `AuthEventPartial`s; the SDK
    /// pairs `Succeeded { connection_id }` with the in-flight `Connection` and surfaces
    /// terminal events to `Library::authenticate_connection`. Default impl returns
    /// `Unsupported`.
    ...
    /// Register a host-resolved credential against the broker's `(BackendId, PrincipalView)`
    /// cache slot, called after the host SDK completes the upstream OAuth flow locally.
...
/// Server-streamed `AuthEventPartial`s while the host SDK drives the interactive OAuth flow.
pub type UpstreamAuthStream =
    std::pin::Pin<Box<dyn futures::Stream<Item = Result<crate::AuthEventPartial>> + Send>>;

/// Wire-shape for `RegisterCredential`; broker pairs it with the in-flight `Auth` keyed by
/// `address`.
#[derive(Clone, Debug)]
