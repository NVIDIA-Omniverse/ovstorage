...
pub use scenarios::{
    CONFORMANCE_ADDRESS_SCHEME, ExpectedCall, FailureContract, Profile, Scenario, ScenarioRegistry,
};

use std::sync::{Arc, Mutex};

use ovstorage_plugin::*;
use ovstorage_plugin::{address, shim};
...
impl shim::Factory for TestFactory {
    ...
    async fn instantiate(
        ...
        let instance = self.shared_instance(cfg.clone());
        let capabilities = cfg.capabilities.clone();
        let backend = Arc::new(TestBackend { instance });
        Ok(shim::BackendInstance {
            backend_id: BackendId(format!("test:{}", cfg.root)),
    ...
    async fn authenticate(
        ...
        let Some(instance) = self.lookup_instance(&root) else {
            // No prior instantiate; emit Succeeded so the host doesn't trip.
...
impl TestBackend {
    ...
    fn relative_key(&self, address: &Url) -> Result<String> {
        let cfg = self.cfg();
        match ovstorage_plugin::address::strip_prefix(address, &cfg.root) {
...
#[async_trait::async_trait]
impl shim::Backend for TestBackend {
    async fn stat(
        &self,
    ...
    async fn read(
        ...
        let key = self.relative_key(&target.resolved_address)?;
        // Returns Err rather than panicking: workspace `panic = "abort"`
        // (FFI ABI stability) makes any in-plugin panic an unrecoverable
        // process abort, so the host-error-handling contract is exercised
        // via Err. Knob name kept for source compat.
    ...
    async fn write(
        ...
        let key = self.relative_key(&target.resolved_address)?;
        if let Some(suffix) = key.strip_prefix(META_PREFIX) {
            if suffix == META_REDIRECT_EXPIRED {
                ...
                let info = StoredObject::new(bytes).info(target.resolved_address);
    ...
    async fn write_redirect(
        ...
        let redirects = build_multipart_redirects(base, &key, parts, cfg.redirect_ttl_seconds);
    ...
    async fn list(
        ...
        let prefix_key = self.relative_key(&prefix.resolved_address)?;
    ...
    async fn watch_address_roots(
        ...
        let _ = &cancel; // conformance plugin: one-shot Snapshot then end.
        self.enter_recorded("watch_address_roots", Some(ObservedCall::WatchAddressRoots))?;
        let cfg = self.cfg();
        let snapshot = AddressRootsChange::Snapshot(vec![AddressRoot {
            address: cfg.root.clone(),
            display_name: None,
            backend_kind: BACKEND_KIND.into(),
            connection_id: None,
        ...
        Ok(Box::pin(futures::stream::iter(vec![Ok(snapshot)])))
    }
    ...
    async fn create_directory(
        ...
        self.enter_recorded(
            ...
            Some(ObservedCall::CreateDirectory {
                target: _target.resolved_address,
            }),
        )?;
        // Directories are implicit in the in-memory store; return a
    ...
    async fn delete_directory(
        ...
        let matching: Vec<String> = store
            .objects
            .range(prefix_key.clone()..)
            .take_while(|(k, _)| k.starts_with(&prefix_key))
            .map(|(k, _)| k.clone())
            .collect();
        if matching.is_empty() {
            return Ok(());
        }
        if matching.iter().any(|k| k != &prefix_key) {
            return Err(Error::new(
...
fn drive_host_callbacks(connection: &Connection) -> Result<()> {
    ...
    let _round_trip = host.keyring_get(BACKEND_KIND, &connection.id, "auth_drives_test")?;
    host.keyring_delete(BACKEND_KIND, &connection.id, "auth_drives_test")?;
...
ovstorage_plugin::ovstorage_plugin!(TestFactory::default, test_only);
