...
fn normalize_grpc_uri(raw: &str) -> String {
...
fn strip_scheme_ci<'a>(s: &'a str, scheme_lower: &str) -> Option<&'a str> {
    let prefix_len = scheme_lower.len();
    if s.len() >= prefix_len && s[..prefix_len].eq_ignore_ascii_case(scheme_lower) {
        Some(&s[prefix_len..])
    } else {
        None
    }
...
mod tests {
    ...
    /// Bare authority with a public host class → TLS. The previous
